...
- `agent start` refuses a provider whose executable is not on `$PATH` before it creates anything — `CliError(ENV)`, exit 3. `src/lib/core/which.ts` is the one `$PATH` probe; `doctor`, `agent status` and `agent start` must not disagree about whether `claude` is installed. Every other bootstrap failure is the provider's to describe: `start` binds four flags and forwards the rest verbatim, so a mistyped yaco flag reaches the provider CLI by design. -> See: [../doc/main/cli/lifecycle.md](../doc/main/cli/lifecycle.md#bootstrap-failure-what-start-can-name-and-what-only-the-provider-can)
