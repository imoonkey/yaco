...
- Package assets (`scripts/agent-wrapper.sh`, `package.json`) and the CLI's own executable resolve through `src/package-root.ts`, never from a checkout, cwd, or `$PATH`. That module must stay exactly one level below the package root in every build layout.
