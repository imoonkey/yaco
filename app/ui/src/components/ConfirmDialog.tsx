...
export function ConfirmDialog({ title, description, confirmLabel, danger, onConfirm, onClose }: {
  ...
  return (
    <DialogShell onClose={onClose} autoFocusRef={confirmRef} className="rounded-xl w-full mx-4" style={{ maxWidth: 360 }}>
      ...
      {description && (
        <div className="px-4 py-3 text-ui-md" style={{ color: 'var(--sol-text)' }}>
          {description}
        </div>
      )}
      ...
      >
        <button
          onClick={onClose}
          className="px-3.5 py-1.5 rounded-md text-ui-md cursor-pointer hover:opacity-85"
        ...
        <button
          ...
          className="px-3.5 py-1.5 rounded-md text-ui-md font-medium cursor-pointer hover:opacity-85"
          style={{
