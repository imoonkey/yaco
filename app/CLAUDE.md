...
- Bundle splits: `Terminal`, `TaskScreen`, `WorkspaceTextSearch`, and the `mermaid` library are loaded via `React.lazy` + `Suspense` (or, for mermaid, the `loadMermaid()` dynamic-import helper in `workspace/markdown.ts`). These surfaces are conditionally rendered, so deferring them keeps the cold-load main bundle small. `FileExplorer` stays a static import — it renders on first paint. `TaskScreen` is shared between `WorkspaceScreen` and `WorkspaceEditorColumn` via a single exported `LazyTaskScreen` so both call sites resolve to one chunk and one Suspense identity.
