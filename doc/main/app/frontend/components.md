...
| `WorktreePicker.tsx` | ~240 | Files-panel worktree selector, mirroring the Changes "Compare ref" behavior (§P2/§P2b/§P2c/§P2d). The non-default state is APP state (`activeWorktree`), not a UI mode: **PRIMARY active → nothing persistent renders** (just a zero-height anchor); **a NON-PRIMARY worktree active → a persistent indicator box** renders atop the Files body AT ALL TIMES (tree AND search mode — it scopes both) showing the branch + dirty/ahead-behind, with an `X` that REMOVES the worktree (`onSelect(null)` → primary, like Compare ref's exit-to-default). The picker is a real floating **`DialogShell` dropdown** (RefSearchDropdown idiom, position:fixed, anchored to the box / host; rows = git-sourced worktrees by branch, `primary` chip, dirty dot, ahead/behind, `Check` on active; no search input). Opened by the header `GitBranch` toggle (any state) or by clicking the indicator (a real `<button>`); the dropdown `autoFocus`es the active option and sets **`restoreFocus={false}`** — a worktree switch does NOT remount the workspace, so restoring focus would leave the header *button* focused after a select and a stray Enter would reopen the picker. Open/close state lives in a module store in `FilesPanel.tsx` (header/body are PanelFrame siblings), mirroring ChangesPanel's `compareSlot`. There is NO header X (the dropdown self-closes; removal is the indicator's X). |
