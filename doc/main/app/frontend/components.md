...
| `WorktreePicker.tsx` | ~190 | Files-panel worktree selector, mirroring the Changes "Compare ref" behavior (§P2…§P2e). The non-default state is APP state (`activeWorktree`), not a UI mode: **PRIMARY active + closed → nothing renders**; **a NON-PRIMARY worktree active + closed → a persistent indicator box** renders atop the Files body AT ALL TIMES (tree AND search mode — it scopes both) showing the branch + dirty/ahead-behind, with an `X` that REMOVES the worktree (`onSelect(null)` → primary, like Compare ref's exit-to-default). The picker renders **INLINE in the panel body** — an accent box (`borderTop: 2px var(--sol-accent)`, tinted bg, `mx-1 mt-1 rounded-md`) like `CompareRefPicker`, pushing the file tree down — NOT a floating overlay. **OPEN** (header `GitBranch` toggle on, any state): the box wraps a `role="listbox"` of git-sourced worktree rows (mono branch, `primary` chip, dirty dot, ahead/behind, `Check` on active; no search input); selecting a row → `onSelect(id|null)` + close. The header toggle (any state) or clicking the indicator (a real `<button>`) opens the list. Open/close state lives in a module store in `FilesPanel.tsx` (header/body are PanelFrame siblings), mirroring ChangesPanel's `compareSlot`. There is NO header X (the toggle closes; removal is the indicator's X). Inline rendering needs no `DialogShell`/`position:fixed`/`autoFocusRef`/`restoreFocus` — closing never restores focus to the header button, so the P2d stray-Enter-reopen regression cannot occur. |
