...
- **Cross-worktree race guard (design §P3)**: every async file fetch (hydration / SSE refetch / per-tab / accept-disk) carries a per-worktree-epoch AbortController **and** captures its `worktreeKey`; a result whose captured key ≠ current is dropped — worktree A's bytes never reconcile into worktree B's view. The captured-key check closes the resolved-then-switched window an abort alone can't.
