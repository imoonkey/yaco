...
- **`useFileState.ts`** — file state stored **per worktree** as `filesByWorktree: Record<worktreeKey, Record<path, FileState>>` (`worktreeKey = activeWorktree ?? projectName`) and **projected to the active worktree's bucket** through `files`/`filesRef` — so every relpath consumer is unchanged. `dirtyTabs`/`conflictTabs`, file CRUD (hydrate, refetch, save, reconcile), and the shared-buffer GC (`gcBuffers`: keep iff referenced by some open editor tab **or** dirty). Uses `fileStateMachine.ts` for explicit state transitions.
...
- **Per-worktree file state (design §P3)**: state lives in `filesByWorktree: Record<worktreeKey, Record<path, FileState>>` (`worktreeKey = activeWorktree ?? projectName`); `files`/`filesRef` project the active bucket, so every relpath consumer (editor, tab bar, save) is unchanged. Switching worktree re-derives the projection from the new bucket and **re-fetches every open tab** for that worktree, so editors show the selected worktree's bytes; a draft in worktree A stays under its key and is restored on return. Saves land in their **captured** worktree's bucket (the worktree read synchronously at save start), so a mid-save switch never strands `saving` nor leaks the result. **Not yet UI-reachable**: `App.tsx` still remounts `<Workspace>` on a worktree switch — the no-remount projection is unit-tested, awaiting the `drop-remount` task.
