...
- **`useFileState.ts`** — file state stored **per worktree** as `filesByWorktree: Record<worktreeKey, Record<path, FileState>>` (`worktreeKey = worktree ?? projectPath`) and **projected to the active worktree's bucket** through `files`/`filesRef` — so every relpath consumer is unchanged. All persisted buckets are seeded at mount, so a worktree switch (no remount) restores its drafts live. `dirtyTabs`/`conflictTabs`, file CRUD (hydrate, refetch, save, reconcile), and the shared-buffer GC (`gcBuffers`: keep iff referenced by some open editor tab **or** dirty). Uses `fileStateMachine.ts` for explicit state transitions.
...
- **`usePersistence.ts`** — two-phase init: takes `(projectName, projectPath)`, returns `initialLayout` + the full `initialDraftsByWorktree` record (every worktree's bucket, the seed `useFileState` restores all buckets from) on mount from localStorage, then `bindSnapshots()` for ref-based debounced save + beforeunload flush. Owns the **layout migration loader** (a stored group blob is normalized as-is restoring `activeGroupId`; an old `panels[]`/leaf tree or the oldest flat blob runs through the pure, idempotent `migrateTreeToGroups` — expand each old editor's `openTabs` into per-file tabs via an old→new id map that re-points `editorMru`; terminal ids + dirty buffers preserved; the old `tasks` tab dropped — then normalize + GC + dedup terminal bindings one-per-session) **and the drafts migration** (`loadDraftsByWorktree` + `commitDraftMigration`). Layout is project-global (`yaco-workspace:<project>`); drafts are one per-project record (`yaco-drafts:<project>`) bucketed by worktree abspath, flushed across all buckets and migrated from legacy `:wt:<slug>`/primary keys on first load. See [persistence.md](../data-model/persistence.md).
...
|-------|------|-----------|
| `panelLayout` | `WorkspacePanelLayout` (group tree + editor-tab payload + instance ids) | localStorage (`yaco-workspace:<project>`, project-global) |
...
| `files` | `Record<path, FileState>` — the **active worktree's bucket** of the per-worktree store | localStorage (`yaco-drafts:<project>`, bucketed by worktree abspath) — dirty drafts only |
...
type FileState = {
  ...
  serverRevision: number | null // revision for the current serverContent
...
- **Shared buffers**: the active bucket is keyed by path, so two editor tabs on one file stay in sync. `gcBuffers` keeps a buffer iff referenced by an open editor tab (`allEditorTabPaths`) or dirty — close/reset never silently loses unsaved work. GC acts on the **active** bucket only, so a background worktree's drafts survive.
- **Per-worktree file state (design §P3)**: state lives in `filesByWorktree: Record<worktreeKey, Record<path, FileState>>` (`worktreeKey = worktree ?? projectPath`); `files`/`filesRef` project the active bucket, so every relpath consumer (editor, tab bar, save) is unchanged. The workspace **no longer remounts** on a worktree switch (P3 drop-remount — `App.tsx` keys `<Workspace>` on `project` only): all persisted buckets are **seeded at mount**, so a switch re-derives the projection from the target bucket — restoring its drafts **live** — and **re-fetches every open tab** for that worktree, so editors show the selected worktree's bytes; a draft in worktree A stays under its key and is restored on return. Seeding every bucket up front is what stops a partially-hydrated background bucket from serializing `{}` and pruning an unopened-path draft (#3b). Saves land in their **captured** worktree's bucket (the worktree read synchronously at save start), so a mid-save switch never strands `saving` nor leaks the result; `acceptDisk` clears immediately only when conflict state already holds a freshly-refetched disk revision (`serverRevision !== baseRevision`), otherwise it waits for the follow-up content fetch before discarding the draft. The fetch uses a **mount-lifetime** AbortController (not the per-worktree epoch), so the accept survives a switch into a background worktree (#3a). The whole `filesByWorktree` is exposed so the drafts-save debounce (`useWorkspaceState`) fires on background-bucket mutations too.
- **Hydration**: on mount **and on every worktree switch**, fetches server content only for open file tabs to detect conflicts.
...
- **Cross-worktree race guard (design §P3)**: hydration / SSE refetch / per-tab fetches carry a per-worktree-epoch AbortController **and** capture the active `worktreeKey`; a result whose captured key ≠ current is dropped — worktree A's bytes never reconcile into worktree B's view. `acceptDisk()` is different: it is an explicit action against a captured bucket and uses a mount-lifetime AbortController, so a worktree switch does not drop the accept. The captured-key check closes the resolved-then-switched window an abort alone can't for view-driven fetches.
...
- **Force save / accept disk**: `forceSave()` writes without revision check; `acceptDisk()` discards local draft from cached disk content only when that cache is known fresh, then reloads server content to catch any newer disk revision.
...
- **Worktree race guard (design §P3)**: root load (`loadRoot`), child loads (`expandDir`), and the SSE refresh all capture their `worktree` and drop a result whose captured worktree ≠ current — on both the success and the failure path (a stale failure never `setError`s on the new worktree nor deletes its `loadedDirs`). The current-worktree ref is updated in a `useLayoutEffect`, so it is current before any fetch callback runs; a per-worktree-epoch AbortController cancels the previous worktree's in-flight loads. With no remount, the load effect also **clears `data` + `loadedDirs` pre-paint (`useLayoutEffect`)** on a worktree switch, so the explorer never renders the old worktree's rows (which a destructive op — delete/move/rename — would otherwise execute against the newly selected worktree).
...
- Active-viewing guard: `visible && document.hasFocus() && attached to target` → the interrupt **still surfaces** (toast + read-back), not suppressed; the guard only auto-acks **when `group==='ready'`** — a viewed REVIEW acks, a viewed ACT (crash/block) is never auto-dismissed (it needs an explicit ✕). Read-back itself is visibility-independent (a hidden tab still speaks).
