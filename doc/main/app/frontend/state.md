...
**Proportional viewport relayout.** Split children store fixed **pixel** `basis`, but the layout keeps every region's *share* of the viewport constant when the window/display changes size (e.g. unplugging an external monitor) — not its absolute px. A provider `useLayoutEffect` runs a rAF-coalesced `ResizeObserver` on the root split that calls `relayoutToViewport(layout, w, h)`: it records the viewport those bases were sized for in a persisted `refSize:{w,h}` and, on a change, multiplies every fixed `basis` by the per-axis size ratio (`scaleFixedBases` — a uniform per-axis scale that **includes hidden** children, so a hidden sidebar is proportional when revealed; distinct from the toggle's `scaleNodeAlongAxis` *redistribution*). It applies synchronously **pre-paint** so a project switch (`Workspace` remounts per `project` — no longer per worktree, since P3 drop-remount) never flashes stale fixed px; `isMobile` is a dep so it re-attaches across the mobile/desktop breakpoint (which swaps the desktop root in place) and skips on mobile. This **replaced** the old per-handle window-resize re-clamp in `usePanelResize` (`maxBasis` now bounds only an active drag). First load of a pre-`refSize` blob adopts its current size as the reference (no historical viewport to scale from), so an old layout saved on a different screen self-corrects only on the next resize, not retroactively.
...
-> See: [hooks.md](hooks.md#decomposed-into) for the reducer API, and `.yaco/plan/all/20260612_panel-vscode-tabs/design.md` for the full model.

## File Buffers — global by path (shared document model)

`useFileState` keys `files`/`dirtyTabs`/`conflictTabs` by **path** (not by tab), so two editor tabs on the same file show the same buffer and the same dirty dot. Only the tab is duplicated; the buffer is one. Internally the state is stored **per worktree** (`filesByWorktree`, keyed by `worktree ?? projectPath` — the worktree abspath, primary = project root path) and projected to the active worktree's bucket, so the path-keyed public surface is unchanged. Switching worktree **no longer remounts** (P3 drop-remount): all persisted buckets are seeded up front, so the switch re-points editors to that worktree's bytes and restores its drafts **live, without a reload**, keeping each worktree's drafts separate. -> See: [hooks.md](hooks.md#key-behaviors) for the projection + cross-worktree race guards.
