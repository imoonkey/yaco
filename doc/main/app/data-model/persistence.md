...
- `panelLayout` — the desktop panel tree (`{ version, desktop, mobile, panelState, refSize? }`); its `tabs` (group) nodes carry the editor-tab payload (`tabId`/`preview`/`pinned`, plus the per-tab md/html view `previewMode`/`splitDirection`/`splitSize`, each omitted when default) and the instance ids the maps key on (editor tabs `editor`/`editor:2…`, terminal tabs `terminal`/`terminal:2…`, groups `group:1…`). `desktop` is canonicalized into `left? · center · right?` regions on every edit (`normalizeRegions`); `panelState` is `{ files: {mode}, separateKinds? }` — `separateKinds` (off by default) persists the kind-affinity open-routing toggle; `refSize:{w,h}` records the viewport the tree's fixed `basis` px were sized for, so a load/resize at a different viewport rescales them proportionally (-> See: [../frontend/state.md](../frontend/state.md))
...
- the md/html view (`previewMode` `'edit'|'preview'|'split'`, `splitDirection` `'horizontal'|'vertical'`, `splitSize` 20–80) is **per-tab** on each editor `GroupTab` (omitted when default `edit`/`horizontal`/`50`), NOT in `layout`
- `layout.autocompleteEnabled` — inline-suggestions toggle (the only editor pref that stays global)
