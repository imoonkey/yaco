...
- **The routed transcript is a one-shot event living in retained state.** `WorkspaceScreen` holds the last confirmed take in `editorInsert` / `terminalSend` and never clears it; each pane de-dupes by the bumped `key` in a ref. A ref only resets on unmount, so a consumer that mounts *after* the event was produced must ignore it — `Editor` / `Terminal` seed that ref with the key present at mount. Without the seed, any remount of the target pane (tab switch back, preview toggle, a transient content reload) replays the take into a fresh CodeMirror view, whose cursor sits at position 0: the text lands a second time at the top of the file.
