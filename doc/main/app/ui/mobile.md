...
On desktop, right-click opens context menus (file explorer, project list, session list, group tabs, dock grips). On mobile, long-press (350ms hold) triggers app-owned row/tab/terminal menus; dock grips are hidden because mobile section order is fixed by the mobile projection metadata. Implemented via the `bind()` method on `useContextMenu()`:
...
- Consumers spread `{...menu.bind(onOpen)}` onto target elements — handles both `onContextMenu` and touch events. Current consumers include file tree rows, project/session rows, desktop group tabs and tab-bar empty areas, mobile editor tab titles, desktop dock grips, and terminal key-bar surfaces.
