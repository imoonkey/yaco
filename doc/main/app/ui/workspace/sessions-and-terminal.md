...
Indentation is `paddingLeft = 8 + depth*14` px on the row, with one dashed vertical guide line per ancestor level (`var(--sol-text-dim)` @ 0.6 opacity) drawn in the indent gutter so the parent→child relationship reads at a glance, mirroring the task-graph tree connectors. Renaming a parent does not rewrite a live child's `parentSession` here — that reference rewrite is owned by the CLI `yaco agent rename` path, not the UI. → See: `doc/main/cli/lifecycle.md` (rename).
