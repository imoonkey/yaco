...
**Optimistic placeholder.** Clicking a New-session button inserts a placeholder row instantly (before the server returns the real handle), keyed by a synthetic `__starting__:<provider>:<n>` name so the list never sits empty during the ~1-3s CLI cold-start. The row shows a friendly **"Starting…"** label (not the raw synthetic id) and suppresses kill/rename/pin actions (the pin keeps its layout box but is invisible+inert, so the row stays aligned with normal rows). It reconciles into the real session row once the handle lands, or ages out (TTL) if the start never materializes. Placeholders are detected by the name prefix, not status, so a *real* server session reporting `starting` with a proper name still renders normally. Because the sessions-dir watcher can surface the real session in the list *before* the start POST resolves with the handle (so it can't yet be matched by name), a still-nameless placeholder is also retired when a session **newly appears for the same provider** — identity-free correlation that prevents a brief "placeholder + real row" duplicate (provider-coarse by necessity; baselined off a real non-null snapshot, reset across project switches). -> See: `useWorkspaceSessions.ts` (`STARTING_SESSION_PREFIX`, `pendingStarts`, `seenSessions`).
...
2. Serve summaries from an in-process cache keyed by `(provider, sessionId, sessionPath)`. The misses — from however many projects — are one `readSessionSummaries` call carrying exactly those sessions, run in this process through `yaco-cli/core/agent/summaries`; sentinel `pending:awaiting-first-prompt` ids are never resolved.
3. The CLI owns all provider-native reconstruction, in the same function `yaco agent summaries` runs. A summary is the **first meaningful** user message, not the literal first JSONL entry: harness `<system-reminder>` blocks and command stdout are dropped, slash commands are restored to `/name args`, and session-management commands (`/rename`, `/clear`, `/compact`) plus handle echoes are skipped. Claude scans its project JSONL; Codex prefers `state_5.sqlite` `first_user_message` over the `title` column (Codex auto-renames the title to the YACO handle on every start, so the title is a name echo), with a rollout-file fallback. app/server never opens `~/.claude` or `~/.codex` itself. → See: `doc/main/cli/providers.md#session-summaries`.

> Summaries are cached in-process by `(provider, sessionId, sessionPath)` and only re-resolve on a `processing → idle` transition or session mutation. The read is a library call now, so a stale label survives a CLI rebuild *and* a CLI edit — restart the server (it runs under `tsx watch`, so touching `app/server/src/**` triggers a reload).
...
The History tab calls `GET /api/sessions/history?project=<name>` and renders the list returned by the server. The server gets rows from `yaco-cli`'s `readProjectHistory`, called in its own process, which returns a windowed object `{rows, returned, truncated, oldestUpdatedAt}` (sorting, the default 200-row `--limit` and the per-provider cap are CLI-owned). A row belongs to the project when its cwd is the project path **or below it** — the same subtree rule the Live tab applies — so an agent that ran in `<project>/.yaco/worktrees/<slug>` stays in the list once it is only history, and its `gitBranch` (`task/<slug>`) is what identifies it. -> See: [../../../cli/read-path.md](../../../cli/read-path.md#a-project-is-a-subtree). `getHistory` maps the CLI shape to the UI shape (`sessionId` → `id`, `updatedAt` → `modified`), tagging `liveSessionName` by matching CLI `sessionId` against the live YACO session list. Provider-native reads and timestamp logic (Claude embedded JSONL timestamps, Codex thread timestamps) live in the CLI provider adapters. The row meta line shows `gitBranch` only when it is not the default branch (`main`/`master`), and a compact `· N tok` (the row's `tokens`, last-turn total token count, via `formatTokens`) when present. → See: `doc/main/cli/providers.md`.
...
- History search filters the already-loaded history rows by title, summary, provider, id, branch, and live-session handle. Timestamps and token counts are intentionally excluded from matching because they create noisy hits for handle-like queries such as `live-7`. Sorting and the default 200-row `--limit` remain CLI/server-owned.
