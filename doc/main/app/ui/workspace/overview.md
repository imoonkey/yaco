...
- **Split / open-to-side / close.** Left-click a group's **split right** or **split down** icon to spawn an adjacent group in that direction, **seeded from the active tab** (an editor tab is duplicated, a terminal tab is moved; an empty source yields an empty group), which becomes the open target. Right-click / long-press either split icon, the tab-bar empty area, or a tab title for the full **Split Up/Down/Left/Right** menu. Plus `Cmd+\` (split the active group along its geometry axis), `Cmd+K Cmd+\` (orthogonal), `Cmd+Enter` (open file to an empty side group), within-group drag-reorder, and `Cmd+W` (close the focused tab, or an empty non-last group). -> See: [../keyboard.md](../keyboard.md).
- **Context menus.** Right-click / long-press a tab title for tab-local actions (`Save`, `Close`, `Close Without Saving`) followed by a divider and group Split actions. Right-click / long-press split icons or the tab-bar empty area for group Split actions; right-click / long-press a dock grip for the recovery-only `Reset layout` action. All app-owned context menu targets use the shared native-callout suppression marker for iOS.
