...
- **Context menus.** Right-click / long-press a tab title for tab-local actions (`Save`, `Close`, `Close Without Saving`) followed by a divider and group Split actions. Right-click / long-press a dock grip for the recovery-only `Reset layout` action. All app-owned context menu targets use the shared native-callout suppression marker for iOS.
