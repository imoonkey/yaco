...
The 3-icon Edit / Split / Preview toggle (or `Cmd+Shift+V` to cycle the active tab) appears in the tab bar for **previewable** files — currently `.md`, `.markdown`, `.mdx`, `.html`, `.htm`, `.csv`, `.tsv` (see `isPreviewableFile` in `ui/src/lib/binaryFiles.ts`). Mode is **per-tab**, carried on the editor `GroupTab` (`previewMode`/`splitDirection`/`splitSize`).
...
- **`/discuss` Q&A tinting**: a custom `renderer.blockquote` in `markdown.ts` tags blockquotes whose leading marker is `**Q{n}**` or `**A{n}**` (the `/discuss` skill convention) with `discuss-q` / `discuss-a` classes, plus `discuss-new` when the marker carries `<mark>` (the newest round). `.markdown-preview` washes questions blue and replies cyan, and renders the newest round as a page-background cutout with a contrasting magenta rim, so multi-round in-doc Q&A is scannable from the preview. Plain blockquotes match nothing and keep the default style; the markdown source stays plain `>` / `>>`.
...
`DelimitedPreview` (`ui/src/workspace/DelimitedPreview.tsx`) renders `.csv`/`.tsv` as a spreadsheet-style table. Parsing is `papaparse` (`delimitedTable.ts`), which handles quoted fields containing the delimiter or a newline, CRLF, and BOM; `.tsv` is pinned to tab while `.csv` uses Papa's delimiter sniffing, so semicolon-separated exports read correctly. Papa's per-row `errors` are recovered warnings and are not surfaced — whatever parsed is previewed, and a file yielding no columns shows a "No rows to preview" placeholder.

The first row is the header; rows are squared to the widest row so a ragged file never shifts cells into the wrong column. Column widths come from the longest value in the first 200 rows, clamped to 6–48 `ch`. The base font size (`--text-ui-xl`, matching the markdown preview) sits on the outer wrapper so those `ch` widths resolve against the same font the cells render in. Rows are banded by display position, mixing `--sol-muted` into the editor background rather than washing the translucent `--sol-subtle-bg` over it, so both row colours are opaque.

**Scroll cost** is the design constraint, since the virtualizer re-renders its whole window on every scroll frame and React Compiler skips this component (both TanStack hooks return functions it cannot memoize safely). Two things follow. Rows render through a `memo`'d `DelimitedRow` taking only primitives — `start` and `banded` derive from an index that does not move for a given row, so scrolling reconciles only the rows genuinely entering the window rather than all ~50 rows × N cells. And the row-number gutter is deliberately **not** frozen: `position: sticky` plus a stacking context on every rendered row measured, at 4× CPU throttle over 5 trials, at p95 31.5ms vs 25.8ms and double the dropped frames. Measured together on a 3000×12 file, vertical scroll went from p50 25.8ms / p95 37.3ms / 12-of-90 frames over 32ms to p50 17.0 / p95 25.8 / 2-of-90. The memo was the dominant win; `overscan` also dropped 16 → 6. Restoring a frozen gutter means one sticky overlay column outside the row markup, not sticky cells per row.

**Text selection**: `DesktopPanelTreeLayout` puts `select-none` on the whole panel tree so its hand-rolled pane drags don't paint a selection, and that inherits into every panel. A reading surface has to opt back in or its text cannot be selected or copied at all — this preview sets `user-select: text` on its root the way `TerminalPanel` does, and `.markdown-preview` carries the same opt-in in `index.css` for the same reason. It reverts to `none` mid column-drag so resizing doesn't smear a selection across cells.

**Overflow** is clip-or-wrap, toggled from the status bar and defaulting to clip. Clipped cells cut off with an ellipsis and carry the full value as a `title` tooltip; wrapped cells drop the tooltip (the text is already on screen) and use `pre-wrap` + `overflow-wrap: anywhere`. Wrapping makes row heights unknowable ahead of layout, so in that mode each rendered row takes `virtualizer.measureElement` as its ref and the fixed `height` comes off; toggling calls `virtualizer.measure()` to void the cached sizes, and the ResizeObserver behind `measureElement` corrects the rest as rows render.

**Column widths** are drag-adjustable from a 6px grip on each header's right edge (pointer events, so it works for touch; double-click hands the column back to auto). Overrides live in a `col → px` map and the grid template mixes units — `120px` for a dragged column, `24ch` for an untouched one — which is why this does not use react-table's `columnSizing`: that API is px-only and would need the `ch` auto-widths converted to px for seed values. Overrides reset when the file changes.

Both the wrap mode and the column widths are view state local to the component; they are not carried on the tab like `previewMode` is, so they reset when the tab is closed.

**Sorting** comes from `@tanstack/react-table` — headless, so it contributes the row model only and the markup/styling stay ours. Clicking a header button cycles asc → desc → unsorted; shift-click adds a sort level. Every column uses the `alphanumeric` sorting function: CSV values are all strings, and it compares embedded numbers numerically so `"99"` sorts below `"100"`. Sorting spans the whole file, not the rendered window (the sorted row model feeds the virtualizer), and resets the scroll offset so a new order doesn't strand the reader mid-file. The row-number gutter shows `row.index + 1` — the row's position in the **file**, not the display position, so a sorted view still points back at the source line. `aria-sort` is mirrored on each `columnheader`.

Rows are windowed with `@tanstack/react-virtual`, so a file at the 1 MB cap costs the same to scroll as a small one. Virtualization needs absolutely-positioned rows, which native table layout cannot express, so the table elements carry `display: grid` — that drops implicit table semantics, hence the explicit `role="table"/"row"/"columnheader"/"rowheader"/"cell"` on every element. The header row is `position: sticky` and the row-number gutter is `sticky left: 0`.

There is no scroll sync for CSV (no source-line anchors); split mode shows two independently scrolling panes.
