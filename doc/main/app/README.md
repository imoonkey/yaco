...
9. **Voice / compose input** → a unified compose tray (type, paste, or record). Entry points: the header **mic** starts a take immediately (same as F5), and the mobile key-bar **launcher** opens the empty tray for typing/pasting. Every recording keeps one native `MediaRecorder` Blob. A Codex take also streams parallel PCM16 through same-origin `/ws/voice/codex`; the server identifies the hidden upstream request as `codex-desktop` and accepts its final-only streaming or buffered provider mode. A non-empty stream final is the raw transcript, while an unavailable, failed, or empty stream automatically sends the cached Blob to the existing Codex batch route **once**. This is same-provider transport fallback, never silent provider failover; Groq remains whole-take batch. Stop/F5 ends the take → optional `/api/voice/format` → text **inserted at the caret** in the draft for review → Insert into editor or terminal. The browser sees neither Codex credentials nor the private upstream protocol: only the YACO server reads the existing Codex-owned OAuth file, without refreshing or writing it. Both Codex streaming and batch endpoints are hidden implementation details, not officially supported public interfaces, so drift is expected to fail closed into batch or the existing Retry state. Provider and **Auto format** are independent persisted preferences; Auto format runs only after the accepted raw transcript. **Retry** reuses the Blob with the provider the user then explicitly selects. A **Format** button re-runs the formatter over the whole draft (with a flat **Undo** button beside it to revert). Send is **⌘/Ctrl+Enter** (plain Enter is a newline, so IME candidate-selection Enter can't mis-send); the tray closes only via the **X / Esc** (never an outside click). Terminal Insert uses the terminal `text-paste` path, so Claude/Codex receive the composed text as one bracketed paste without auto-submit. The clipboard is written only by the explicit **Copy** button — closing the tray never touches it. Config: optional `CODEX_HOME`, `GROQ_API_KEY`, and `VOICE_FORMATTER_MODELS` in `server/.env`.
...
Design doc: [.yaco/plan/all/markdown-inline-suggestions/final/design.md](../../../.yaco/plan/all/markdown-inline-suggestions/final/design.md).
