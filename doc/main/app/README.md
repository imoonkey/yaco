...
12. **Multi-instance panels** → the workspace holds N editor + N terminal panes. One reducer (`useLayoutState.ts`) owns the desktop panel tree + per-instance `editorViews`/`terminalBindings`/MRU/`focusedPane`; the tree is authoritative and the maps GC against it. File buffers stay global by path (`useFileState`, shared document model). Type-global commands route to the active instance (MRU head → first in order); `PanelHost` publishes each pane's `instanceId`; per-session miss-count reconcile closes panes for dead sessions; `visibleSessions` drives unread. Persisted per (project, worktree). Behavior spec: [ui/workspace/](ui/workspace/), state model: [frontend/state.md](frontend/state.md#workspace-hot-state--one-reducer-multi-instance), design: [plan/all/20260611_panel-multi-instance/design.md](../../../plan/all/20260611_panel-multi-instance/design.md).
