...
- The `.gitignore` is loaded **before** the watcher is created so the first walk already prunes; directory-only patterns (`dist/`) are matched via chokidar's stats arg + a trailing slash. The `.worktrees` container stays watched (gitignored though it is) so the `worktrees` channel and per-worktree `filetree` refresh still fire. Paths below `.worktrees/<slug>/` are canonicalized back to repo-relative paths before applying the root `.gitignore`, so ignored trees such as `data/` and `.venv/` do not multiply across every worktree; `node_modules`, `logs/{traffic,usage}`, and `.git/{objects,logs,index.lock}` remain hard-pruned by segment. `watchProject()` awaits chokidar `ready` (bounded) so writes are observed once it resolves, and guards watch/unwatch races with a per-path generation token. The global session watcher still uses `fs.watch` on `${YACO_HOME:-~/.yaco}/sessions` (`constants.AGENT_SESSIONS_DIR` → `sessionsDir()`)
