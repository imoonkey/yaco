...
- Fixed project layout (`project.ts`): repo-relative constants `PLAN_DIR = ".yaco/plan"`, `TASKS_DIR = ".yaco/plan/tasks"`, `WORKTREES_DIR = ".yaco/worktrees"` (callers `join(repoRoot, CONST)`; no config file), plus `resolveDocDir(repoRoot)` — first existing dir of `docs/`, `doc/`, else `docs/`. Server consumers: `attention-runtime.ts` + `project-watcher.ts` (`TASKS_DIR`), `project-watcher.ts` + `routes/sessions.ts` (`WORKTREES_DIR`), `colocatedRepos.ts` (`PLAN_DIR`). Project identity lives only in `~/.yaco/projects.json`.
