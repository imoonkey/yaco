...
- Shell and agent terminal views both attach to tmux via `tmux attach-session` through node-pty, with the PTY carrying the client's `cols`/`rows`. Before spawning it, `attachSession()` sets `window-size smallest` on the session, so tmux recomputes the window from every attached client on each attach, detach, and resize. No explicit `resize-window` is issued — tmux documents that it flips `window-size` to `manual`, which would defeat the policy. The option is set **per session** (not on the server global) because a session-local value, which sessions created by earlier builds carry, wins over the global.
- `smallest` over `latest`: with two devices on one session (laptop + phone), `latest` resizes the window on every switch of the active client, and each resize costs a full-screen repaint pushed to **both** clients plus a TUI relayout — a continuous repaint storm. `smallest` fits the window to the narrowest client and only changes on attach/detach/resize, trading a letterboxed laptop view for a stable one.
