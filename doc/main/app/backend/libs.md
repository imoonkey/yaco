...
The runtime-root helpers and the fixed project layout live in the workspace package `yaco-cli` (TypeScript source under `cli/src/lib/core/paths/`) and are consumed by `app/server` via the workspace exports map. The local `yacoHome.ts` and `yacoPaths.ts` modules were deleted in the same pass — there is exactly one resolver implementation in the monorepo.
...
- Fixed project layout (`project.ts`): repo-relative constants `PLAN_DIR = ".yaco/plan"`, `TASKS_DIR = ".yaco/plan/tasks"`, `WORKTREES_DIR = ".yaco/worktrees"` (callers `join(repoRoot, CONST)`; no config file), plus `resolveDocsDir(repoRoot)` — the `docs/` folder (an existing `doc/` counts instead). Server consumers: `attention-runtime.ts` + `project-watcher.ts` (`TASKS_DIR`), `project-watcher.ts` + `routes/sessions.ts` (`WORKTREES_DIR`), `colocatedRepos.ts` (`PLAN_DIR`). Project identity lives only in `~/.yaco/projects.json`.
- Project registry (`project-registry.ts`): `readProjects()`, `writeProjects()`, `addProject()`, `removeProject()`, `projectsRegistryPath()`, `ensureYacoHome()` — sync I/O and validated registry mutation helpers. `addProject()` rejects the home directory (`INVALID`): a project's `.yaco/` there would sit on top of the runtime home.
...
**CLI surface:** `yaco paths runtime [--json]` returns the runtime helpers keyed by name. `yaco paths project [--json] [--repo <path>]` returns `{ plan, tasks, worktrees, doc }` **resolved to absolute paths** against `--repo` (defaults to cwd). `--repo` with no value is rejected as `USAGE` (exit 2). -> See: [cli/paths.md](../../cli/paths.md)
...
**Exports**: `readSessionsFromStateFiles()`, `readAllSessionsFromStateFiles()`, `fetchAllSessionsFromCli()`, `queryAgentStatus()`, `inspectSessionMessages()`, `sendToSession()`, `captureSession()`, `startAgentSession()`, `closeAgentSession()`, `renameAgentSession()`, `AgentSession`, `AgentSessionState`, `isPathDescendantOrEqual()`

**Five reads are no longer in this contract** — the task graph, session-list labels, the provider catalog, the channel `/last` message read and the project history read call `yaco-cli` exports in this process. Everything below still spawns. -> See: [../../cli/read-path.md](../../cli/read-path.md)
...
- `getHistory(projectPath, liveSessions)` — calls `yaco-cli/core/agent`'s `readProjectHistory` **in process** (no spawn), passing the live sessions it already holds, then maps each CLI row to the UI shape (`sessionId` → `id`, `updatedAt` → `modified`) and tags `liveSessionName` by matching CLI `sessionId` against the live `AgentSession[]` (skipping `pending:awaiting-first-prompt`). Sorting, the default 200-row `--limit` and the per-provider cap are CLI-owned. A `Result` failure is raised as `yaco agent history failed [CODE]: message`; the route has no handler, so the response is the same `500 "Internal Server Error"` the spawn produced. -> See: [../../cli/read-path.md](../../cli/read-path.md#5--history-tab--readprojecthistory)
...
- The move is measured, not assumed, and the reader had to be rewritten to earn it: as it stood it starved an already-queued timer to p95 64 ms against the spawn's 27 ms, and only a form that caps every provider scan at the window and yields between chunks came in under the bound, at 12.4 ms. -> See: [../../cli/read-path.md](../../cli/read-path.md#5--history-tab--readprojecthistory), [../../cli/exports.md](../../cli/exports.md#the-queries-rule-5-has-judged).
...
- `listRegisteredWorktrees(primaryRoot)` — runs `git worktree list --porcelain` once and parses every entry (primary + linked, including worktrees outside `.yaco/worktrees/`); returns `[]` if git fails. The shared lister behind both the worktrees route and the badge helpers.
...
- `getWorktreeStatus(projectPath, slug)` — verifies `.yaco/worktrees/<slug>/` (`worktreePath(projectPath, slug)` from `yaco-cli/core/worktree`) is a registered git worktree via `listRegisteredWorktrees` (not just `existsSync` — prevents stale directories from appearing active), then resolves its `worktreeStatus`. Returns inactive status if not registered.
...
- `extractWorktreeSlug(sessionPath, worktreesPath)` — the first path segment of `sessionPath` below `worktreesPath` (`<project>/.yaco/worktrees`), else `undefined`. Used by the sessions route to tag agent sessions with their worktree.
...
Detects **colocated repos** — directories that are their own git repo but kept out of the host repo (motivating case: a private `.yaco/plan` excluded via `.git/info/exclude`).

**Exports**: `getColocatedRepos(projectPath)`, `clearColocatedReposCache()`

- Candidates: every depth-1 child whose `.git` exists (dir **or** worktree file; self/ancestor symlinks like `loop -> .` are skipped), plus `.yaco/plan` (`PLAN_DIR`) when `.yaco/plan/.git` exists.
- A candidate is detected when **nothing at or under it is in the host index** (one `git ls-files -z` read) and it is **not matched by the root working-tree `.gitignore`** (reuses `gitignore.ts` — the same source the tree's dimming uses, so detection and dimming never disagree). Returns repo-relative paths, sorted. No policy knob: detection is the whole rule.
...
- **`channels/pty-tap.ts` + `channels/pane-reply.ts`** — fallback when a session has no output cursor. A ref-counted `tmux pipe-pane` tap drains the PTY into a bounded ring and exposes `waitForQuiet`. For Codex, the ring is timing-only: the router reads `tmux capture-pane`, whose screen model has already applied cursor movement and erase commands, and `pane-reply.ts` extracts output between the submitted prompt and the next input prompt. Spinner repaints and terminal controls therefore never become channel text. Other providers retain the offset/slice fallback because they do not share Codex's prompt framing. `/last` still reads JSONL; `/capture` remains the explicit full-pane debug surface.
