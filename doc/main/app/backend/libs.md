...
- Repo-relative reader (`yaco-paths.ts`): `readYacoProjectPaths(repoRoot)` returns the canonical four paths (`tasks`, `active`, `archive`, `worktrees`) merging optional `yaco.toml [paths]` overrides over the defaults. Defaults: `projects/tasks.json`, `projects/active`, `projects/archive`, `.worktrees`. Project identity lives only in `~/.yaco/projects.json`, so `[project]` is never read.
