...
6. Subscribe to PTY output on the microtask that resumes the `await attachSession(...)`. That still precedes every PTY I/O turn, so tmux's attach repaint (~30ms after the spawn) and its capability queries land on this subscription. `attachSession` guarantees the gap by awaiting nothing between `pty.spawn()` and its return; there is no startup buffer, and no scrollback either — shell and agent scrollback is tmux-managed. -> See: [libs.md#terminalts-460-lines](./libs.md#terminalts-460-lines)
...
12. Handle image-paste messages (`{ type: 'image-paste', mime, base64 }`) by writing the bytes into the X11 CLIPBOARD via `clipboard-write.ts` (which resolves only once the image is verified readable back), then forwarding `\x16` (Ctrl+V) so the focused TUI agent (Claude Code, Codex) reads it through its native paste path. Pastes are **serialized per session** and the queue is held for a short window after Ctrl+V, so a rapid re-paste can't replace the clipboard owner while the agent is still reading it — that ownership churn is what dropped the agent onto the hanging `wl-paste` fallback ("Pasting…" freeze). Used to forward laptop-clipboard images into a remote desktop's TUI agent — see `lib/clipboard-write.ts` and `lib/clipboard-env.ts` in [libs.md](./libs.md).
...
`MediaRecorder` (`app/ui/src/hooks/voiceCapture.ts`). The UI build also emits
`codexPcmProcessor.ts` as a roughly 1 KB JavaScript asset; a browser fetches and
instantiates that AudioWorklet only when a caller supplies a PCM sink. It
converts mono input into bounded PCM16 frames. The one-argument capture path
fetches no worklet and creates no `AudioContext`; worklet failure therefore
does not disable MediaRecorder capture.
...
| `VOICE_SPEAK_MODELS` | unset | Comma-separated Groq models for the spoken paraphrase (quality-first); falls back to a built-in chain led by `openai/gpt-oss-120b` (small/fast models translate 中文→English) |
