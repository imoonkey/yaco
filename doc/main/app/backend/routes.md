...
| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/files/:project` | Root-level file listing (lazy — dirs have `children: []`, gitignored entries marked, colocated-repo dirs flagged `colocated: true`) |
...
| GET | `/api/files/:project/children?dir=...` | One directory's immediate children (lazy expand on demand). Flags colocated-repo dirs the same way at any depth, so `?dir=.yaco` returns `plan` with `colocated: true` |
...
| GET | `/api/git/:project/diff?path=...` | Unified diff for a file. Optional `&base=REF&compare=REF` for ref-to-ref diff (host-rooted); without them falls back through: `git diff HEAD` → `git diff --cached HEAD` (staged) → `--no-index /dev/null` (untracked), run in the file's **owning repo** via `resolveFileRepo(..., "deny")` — a colocated logical path (`.yaco/plan/foo.md`, path-boundary so `.yaco/plan2/x` ≠ `.yaco/plan`) diffs against that repo's HEAD (works for deleted files); a host symlink is never followed outside the project (git never runs in an external location) |
...
A **colocated repo** is a directory that is its own git repo but is deliberately kept out of the host repo (the motivating case: a private `.yaco/plan`, excluded via `.git/info/exclude` by [`yaco plan init`](../../cli/plan.md)). `lib/colocatedRepos.ts` `getColocatedRepos(projectPath)` takes as candidates every depth-1 child whose `.git` exists (dir or worktree file) plus `.yaco/plan` when `.yaco/plan/.git` exists, and keeps a candidate when **nothing at or under it is in the host index** (one `git ls-files -z` read) and it is **not matched by the root working-tree `.gitignore`** (the same source the tree's dimming uses, so detection and dimming never disagree). A tracked `.yaco/plan` (no `.git`) is an ordinary host directory. Result is cached by `realpath(projectPath)` for a short TTL (no watchers). The read-only git surfaces above mirror across the host + each detected repo so a colocated repo shows up first-class (searchable, changes/diffs, undimmed tree) without ever entering host git. -> See: [lib/colocatedRepos.ts](libs.md), [yaco plan init](../../cli/plan.md)
