...
6. **Terminal** → WebSocket `/ws/terminal/:name?project=<projectName>` → node-pty `tmux attach-session`. Shell sessions are Workflow-managed tmux sessions with ownership state in `${YACO_HOME:-~/.yaco}/shell-sessions/<name>.json`, so they survive browser detach and server restart until explicitly killed; agent sessions use the global `~/.multmux/sessions/<handle>.json` state file (multmux root; yc-multmux-state-root moves it under `${YACO_HOME}/sessions/`) where `handle` is the tmux session name
