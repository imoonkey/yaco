...
6. **Terminal** → WebSocket `/ws/terminal/:name?project=<projectName>` → node-pty `tmux attach-session`. Shell sessions are Workflow-managed tmux sessions with ownership state in `${YACO_HOME:-~/.yaco}/shell-sessions/<name>.json`, so they survive browser detach and server restart until explicitly killed; agent sessions use the global `${YACO_HOME:-~/.yaco}/sessions/<handle>.json` state file (multmux root; `MULTMUX_STATE_DIR` env var is an explicit override) where `handle` is the tmux session name
...
9. **Voice input** → browser `MediaRecorder` captures audio → POST `/api/voice/compose` (multipart) → Groq Whisper STT (with bilingual `initial_prompt` conditioning) → multi-model LLM formatter → compose tray for user review → Insert (editor) or Send (terminal). Defensive: any tray close that carries edited content (Insert, Discard, X, Esc) first backs the draft up to the clipboard via `writeTextToClipboard()` (execCommand fallback covers mobile / plain-HTTP LAN), so a glitched insert never silently loses the text. Config: `GROQ_API_KEY` + optional `VOICE_FORMATTER_MODELS` in `server/.env`.
