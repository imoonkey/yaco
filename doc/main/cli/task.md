...
| `index.ts` | Re-exports the **read** surface: `model.ts`, `validation.ts`, `graph.ts`, and the loads from `store.ts` | The published `@yaco/cli/core/task`. Reads go through this barrel; a writer, the lock or a link mutation is imported from its own module by `cli/src/commands/task/*` — the export audit fails if one re-enters the barrel. |
