...
The unit is a singleton, so concurrent starts race for it. The loser retries **unwrapped, and only once a server is confirmed running** — that server is the winner's, already escaped, and joining it is correct. If no server appeared, the failure was the escape's own (no user bus, an unsupported option) and the error is raised rather than silently founding the server inside the restartable service.
