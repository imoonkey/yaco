...
1. **Hook-based (primary)**: Claude Code (12 events) and Codex (8 events) fire lifecycle hooks (SessionStart, UserPromptSubmit, Stop/StopFailure, PreToolUse/PostToolUse/PostToolUseFailure, PermissionRequest, Notification, PreCompact/PostCompact, SessionEnd — provider availability varies, see [providers.md](providers.md#hook-availability)). Provider configs point at `<absolute-yaco-executable> agent hook-event <EventName>`. The handler reads the event JSON from stdin, identifies which session the calling process belongs to via `resolveWhoamiMatch` (own pane `$TMUX_PANE` → provider session-id env → ancestor PIDs, and only ever a handle that owns a live state file; no match → the event is dropped), then runs `runHookEventForHandle`, which applies `applyHookEvent` to compute the next state and writes via the same atomic temp-file-rename writer. Identity must come from the calling process rather than from ambient tmux state: those configs are installed **globally**, so this entry point runs for every provider process on the machine, and an untargeted `tmux display-message -p '#{session_name}'` answers even outside tmux — with the server's most-recently-active session, which applies a foreign process's event to an unrelated live agent. -> See: [state-contract.md](state-contract.md#yaco-agent-hook-event-eventname) **Note:** Codex lacks `SessionEnd` and `StopFailure` hooks — see exit-trap wrapper below. **Context reset safety:** `SessionEnd` sets status to `idle` (not delete), because Claude fires `SessionEnd` → `SessionStart` on context window resets while the process is still alive. Actual file deletion happens only via the wrapper EXIT trap.
...
The CLI surfaces `app/server` still **spawns** (all `--json` except the NDJSON
stream):
...
`cgroupEscapePrefix()` in `src/lib/core/agent/tmux.ts` detects this case (Linux + `systemd-run` available + leaf cgroup ends in `.service` and isn't `user@<uid>.service`) and prefixes `tmux new-session` with `systemd-run --user --scope --unit=yaco-tmux-server`. tmux ends up in a transient `.scope` outside the parent's control-group; the parent restart leaves it alone.

**The escape has one destination, because tmux has one server.** Only the invocation that finds no server actually starts one; every later `new-session` is handled by that existing server, which forks the pane into *its* cgroup whatever scope the requesting client ran in. So the escape is applied to the server-starting invocation alone (`isTmuxServerRunning()` gates it) and names a fixed unit. Wrapping each `new-session` in an anonymous `run-p<pid>-i<id>.scope` bought no per-session isolation — it only decided which session's scope the whole fleet would live in, and billed that scope's entire footprint to that session's command line in `systemctl` and in the journal's `Consumed … CPU time` line. `--description` is set for the same reason: systemd otherwise defaults it to the first session's command line.

The unit is a singleton, so concurrent starts race for it. The loser retries, but only on evidence that the retry is the right move: the session is **confirmed** absent (`checkSessionAlive`'s tri-state — a probe that could not answer is not an absence) and a server is running. That server is the winner's, already escaped, and joining it is correct. If no server appeared, the failure was the escape's own (no user bus, an unsupported option) and the error is raised rather than silently founding the server inside the restartable service. The retry itself carries `tmux -N`, which forbids starting a server: should the winner's last session end in the window between the check and the retry, the retry fails instead of founding an unescaped replacement.

One fixed unit name per user account is sound because yaco always talks to one tmux server: it never passes `-L`/`-S`, and when `$TMUX` selects another socket a server is by definition already running there, so no escape is attempted. Adding a socket-selection flag would make the unit name need the socket in it.
