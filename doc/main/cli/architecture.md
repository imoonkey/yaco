...
- **Hook command — canonical form.** Resolved once at install time via `hookBinary()` → `<absolute-yaco-binary> agent hook-event <Event>`. Resolution order: `$YACO_BIN_DIR/yaco` → `process.execPath` when this process *is* the yaco executable (`package-root.ts#selfExecutablePath`) → `which yaco` → literal `"yaco"`. Absolute paths ensure the hook works even when the tmux server / provider runs with a stripped PATH. A runtime plus a source path is never emitted: neither the runtime nor the checkout is guaranteed to be reachable at hook-fire time, and a `tools/install.sh` that put the artifact in `$BIN_DIR` and then moved the repo away breaks both.
