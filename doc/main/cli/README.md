...
6. **[paths.md](paths.md)** — Path resolvers under `cli/src/lib/core/paths/` (exported as `yaco-cli/core/paths`): runtime root, the fixed `.yaco/` project layout (`PLAN_DIR`/`TASKS_DIR`/`WORKTREES_DIR`) + `resolveDocDir`, project registry, `project current` cwd→owner resolve, `yaco paths` CLI surface.
