...
6. **[paths.md](paths.md)** — Path resolvers under `cli/src/lib/core/paths/` (exported as `yaco-cli/core/paths`): runtime root, the fixed `.yaco/` project layout (`PLAN_DIR`/`TASKS_DIR`/`WORKTREES_DIR`) + `resolveDocsDir`, project registry, `project current` cwd→owner resolve, `yaco paths` CLI surface.
...
10. **[worktree.md](worktree.md)** — `yaco worktree {create,merge,cleanup}` surface and `cli/src/lib/core/worktree/`: slug-keyed `.yaco/worktrees/<slug>` provisioning (plan linked per its privacy state), the shared slug↔path↔branch convention export, rebase + ff-merge or PR mode, conservative cleanup — TS port of the orchestrate shell helpers.
...
15. **[plan.md](plan.md)** — `yaco plan init` surface and `cli/src/commands/plan/`: promote `.yaco/plan` into a private colocated git repo (in-place `git init`, `/.yaco/plan` in the git-resolved `info/exclude`, `!.yaco/` + `!.yaco/plan/` in root `.ignore`, idempotent, `--remote` adds origin but never pushes). CLI side of the app's [colocated-repos](../app/backend/routes.md#colocated-repos) mechanism.
