...
8. **[read-path.md](read-path.md)** — The read/lifecycle split: the three conditions that admit a CLI read into `app/server`'s process, the four landed cutovers with their measured before/after and accepted limits, why the history read was measured and left a subprocess, what still spawns, and how to roll any one route back.
