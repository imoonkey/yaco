...
**Why:**
- Workflow's [yaco-core design](../../workflow/projects/active/yaco-core/final/design.md) §Canonical Path Layout consolidates runtime state under `${YACO_HOME:-~/.yaco}/`. Adding the Python-side resolver in lockstep with the TS resolvers (workflow + multmux) means any new skill script that needs the runtime root has one place to read from and one set of tests it must match.
