...
**Why:**
- Workflow's [yaco-core design](../../workflow/projects/active/yaco-core/final/design.md) §Canonical Path Layout puts the managed hook/wrapper scripts under `~/.yaco/`. Doing the resolver + the two leaf path constants in one pass keeps the eventual sessions-dir move (yc-multmux-state-root) mechanical — flip imports, no derivation. No data migration here: this PR does **not** run `multmux install-hooks`, does not move existing `~/.multmux/hook-v2.sh` or `wrapper-v2.sh`, and does not change the on-disk state-file format.
