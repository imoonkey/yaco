...
- The resolver chains existed because a Bun-compiled binary served its modules from a virtual filesystem and the package could not name its own files. A real package root supplies a floor rung that always exists, so the literal `"yaco"` — which wrote a hook command that failed silently at every fire — has something to be replaced by. QA caught the over-deletion: dropping the `which yaco` rung as well meant `agent start` from a checkout rewrote the machine's global hooks to that checkout's launcher, which dies with the worktree.
- One deliberate behavior change: `npm ci --workspace` prunes every workspace it was not asked about, so the old "repair any shape of interrupted install" branch would have deleted a developer's app dependency tree. The bootstrap now installs only into a clone with no `node_modules` and otherwise reports `npm ci` as the remedy.
