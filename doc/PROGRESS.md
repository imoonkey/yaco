...
- One deliberate behavior change: `npm ci --workspace` prunes every workspace it was not asked about, so the old "repair any shape of interrupted install" branch would have deleted a developer's app dependency tree. The bootstrap now decides from `node_modules/.package-lock.json` — npm's own record of what it installed — repairing a tree that records only the `cli` workspace (or records nothing) and refusing one that records more.
