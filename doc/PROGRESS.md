...
- `package-root.ts#yacoExecutable()` replaces two divergent chains (`lifecycle.ts`, `tmux.ts`) with one: `$YACO_PATH` → `$YACO_BIN_DIR/yaco` → `<package-root>/bin/yaco.mjs`. The wrapper's four-rung checkout walk is gone; `readAgentWrapperScript()` reads the packaged asset or raises `INTERNAL`.
...
- The resolver chains existed because a Bun-compiled binary served its modules from a virtual filesystem and the package could not name its own files. With a real package root every rung was a way to reach a *different* checkout or installation than the one running — and the last one, the literal `"yaco"`, wrote a hook command that failed silently at every fire.
