...
- **Three instrument corrections, each of which had been flattering the result.** (1) The fixture's composite index was named `…_updated_at_ms` but indexed `updated_at`, so the benchmark measured an index-prefix scan where production sorts in a temp B-tree; the fixture now carries Codex's real columns and index set, and the probe prints the plan. (2) The fixture wrote 200 Codex name-index records at *both* scales while the reader splits and scans that whole file — a tenth of the reference home at 1x, a hundredth of what 10x claims; it is now its own measured dimension, 2 013 / 20 130. (3) The benchmark verdict was computed over every in-process form, so it would have printed success whenever any control cleared the bound, including a run where the shipped route did not.
...
- real provider home (11.1 MB `state_5.sqlite`, 2 301 threads, 81 Claude logs) — **32.3 → 13.0 ms** (215 → 116 ms); control 81.5 ms
- synthetic 1x — **38.5 → 14.9 ms** (165 → 91 ms); control 51.3 ms
- synthetic 10x — **111.4 → 21.6 ms** (354 → 240 ms); control 584.7 ms
