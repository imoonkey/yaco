...
- `RULE_5_SQLITE` in the export audit is rule 5's first *admission* (permanent, evidenced) as opposed to a *debt* (owed, retired): Codex's `threads` point lookup, isolated in `providers/codex-thread.ts`. It pins the exact SQL through the AST and rejects every *read* of an unbounded execution member in the admitted module — property access rather than call, because review defeated the call-name version three ways in one sitting. That rule is what makes an admitted module necessarily tiny. `summary-stall.ts --sqlite-probe` reproduces the plan and the 0.3 ms cost it cites.
...
- Five High findings across two rounds of independent Codex review changed the code rather than the prose. Chunking bounds the scan but not one record, and a 36 MB record measured 96 ms — hence the cap, and a `--long-record` fixture that holds the route to the same bound. "Strictly more labels" was wrong: the newest rollout can open with nothing but a filtered context block while the real prompt sits in yesterday's, so the resolver became a lazy newest-first generator the summary read walks until a label. And the SQLite admission check was fail-open twice: first a text match that `const s = db.prepare(q); s.all()` walked straight through, then a callee-name match that fell to `s.all.bind(s)()`, `s.all.call(s)`, and a local binding shadowing the `Promise` global it exempted. Rejecting the member read closes all of them, at the price of an admitted module small enough to have no `Promise.all` in it.
...
**Verification:** `bash scripts/verify.sh` all 11 steps pass (1334 CLI unit tests, 859 app/server tests). Label parity on the operator's real provider home: **611/611** sessions (211 Claude logs across every project directory, 400 Codex threads) identical to the implementation this replaces. The CLI golden matrix is unchanged, so `agent summaries` and `agent providers` stdout/stderr/exit code are byte-identical to the pre-cutover baseline. Ten user flows end to end against isolated servers, including a stubbed `yaco` binary that proves the labels no longer come from a subprocess and that `agent start` still spawns. Reverting both feature commits applies cleanly and leaves every suite green. See `plan/all/cli-node-sdk/{qa,review_codex}-summary-read-cutover.md`.
