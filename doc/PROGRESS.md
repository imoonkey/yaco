...
- `RULE_5_SQLITE` in the export audit is rule 5's first *admission* (permanent, evidenced) as opposed to a *debt* (owed, retired): Codex's `threads` point lookup, isolated in `providers/codex-thread.ts`. It pins the exact SQL and the module's import list through the AST, and constrains the module's *syntax* — no destructuring, no `Reflect`/`eval`/`Function`/`Proxy`, no member named `all`/`run`/`exec`/`iterate`/`call`/`bind`/`apply`, no computed member it cannot name. Three successive versions that enumerated dangerous spellings were each defeated in review; an allowlist on a single-purpose module is what ends that race, and it is why the query has a module of its own. `summary-stall.ts --sqlite-probe` reproduces the plan and the 0.3 ms cost it cites.
...
- Six High findings across three rounds of independent Codex review changed the code rather than the prose. Chunking bounds the scan but not one record, and a 36 MB record measured 96 ms — hence the cap, and a `--long-record` fixture that holds the route to the same bound. "Strictly more labels" was wrong: the newest rollout can open with nothing but a filtered context block while the real prompt sits in yesterday's, so the resolver became a lazy newest-first generator the summary read walks until a label. And the SQLite admission check was fail-open three times over: a text match that `const s = db.prepare(q); s.all()` walked through, a callee-name match that fell to `s.all.bind(s)()` and a shadowed `Promise`, and a property-access match that missed `const { all } = statement` and `Reflect.get(statement, "all")` — the shape that runs a whole second unbounded query while the audit reports exactly the admitted one. Enumerating spellings loses that race; constraining a single-purpose module's syntax ends it.

**Key files:** `cli/src/lib/core/agent/{provider-catalog.ts,index.ts}`, `cli/src/lib/core/agent/providers/{summary-read,codex-thread,prompt-label,provider-home,output,history,types,claude,codex}.ts`, `cli/src/commands/agent/{summaries,providers,index}.ts`, `cli/package.json`, `app/server/src/lib/{session-summary,agent}.ts`, `cli/test/{summary,providers,agent-messages-parity}.test.ts`, `cli/test/unit/export-audit.test.ts`, `cli/test/helpers/export-closure.ts`, `cli/test/bench/summary-{stall,fixture}.ts`, `doc/main/cli/{providers,exports}.md`, `doc/main/app/backend/{libs,routes}.md`, `doc/main/app/ui/workspace/sessions-and-terminal.md`, `app/CLAUDE.md`
**Verification:** `bash scripts/verify.sh` all 11 steps pass (1335 CLI unit tests, 859 app/server tests). Label parity on the operator's real provider home: **611/611** sessions (211 Claude logs across every project directory, 400 Codex threads) identical to the implementation this replaces. The CLI golden matrix is unchanged, so `agent summaries` and `agent providers` stdout/stderr/exit code are byte-identical to the pre-cutover baseline. Ten user flows end to end against isolated servers, including a stubbed `yaco` binary that proves the labels no longer come from a subprocess and that `agent start` still spawns. Reverting both feature commits applies cleanly and leaves every suite green. See `plan/all/cli-node-sdk/{qa,review_codex}-summary-read-cutover.md`.
