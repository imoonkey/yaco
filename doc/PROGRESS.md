...
**Verification:** `scripts/verify.sh` green (cli test + server test + ui lint + root build/tsc -b); `app/ui` `npx tsc -b` + `npm run lint` + `npm test` (1074 passed). Cross-provider codex review + `/qa` evidence under `plan/all/worktree-explorer-view/`.
