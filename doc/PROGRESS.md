...
- `RULE_5_SQLITE` in the export audit is rule 5's first *admission* (permanent, evidenced) as opposed to a *debt* (owed, retired): Codex's `threads` point lookup. It pins the exact SQL through the AST and rejects every unbounded statement method in the admitted module; `summary-stall.ts --sqlite-probe` reproduces the plan and the 0.3 ms warm / 1.4-1.8 ms cold cost it cites.
- Two accepted behaviour changes, both recorded in `doc/main/cli/providers.md`: a record over 4 MiB is skipped undecoded, and the Codex rollout search is the whole `YYYY/MM/DD` tree (continuing past a rollout that yields no label) rather than a private eight-day walk.
