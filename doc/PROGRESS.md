...
- **The Claude half is per *log*, not per directory, and that was the review's doing.** The first implementation found candidate directories by encoded-name prefix and confirmed each with the first `cwd` one of its logs recorded. `encodeClaudeCwd` collapses every non-alphanumeric to `-`, so that is unsound in the exact case it defends against: `/repo/demo/a_b` and `/repo/demo-a-b` land in one directory, and so do `/repo/demo` and `/repo:demo`. The cross-provider reviewer built fixtures showing valid history disappearing in one `readdir` order and unrelated history leaking in the other. Attribution moved into `claudeTail`, which parses the cwd out of the tail slice it reads anyway — **exact, order-independent, and no extra I/O**. A log recording no cwd falls back to the directory it was filed under: kept under the project's own exact encoded name, dropped under any prefix guess.
