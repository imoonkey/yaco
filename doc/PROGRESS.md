...
- **The rule is stated as the conjunction it is.** Export eligibility is necessary and not sufficient: a path also has to be a read rather than a mutation, and it has to starve an already-queued request no longer than the complete subprocess route it replaces. The history read is the case that makes the distinction concrete — it passes the audit and is still a subprocess. `doc/main/cli/exports.md` now says so at the top, so passing the audit is not read as permission to move a route.
- **The measured figures are in `doc/`, not only in `plan/`.** Task GET 181.6 → 28.5 ms and the read alone 427.3 → 72.1 ms at 4 850 tasks; summaries 122 → 30 ms real home and 138 → 36 ms at ten times, starvation 27.1 → 13.4 and 34.3 → 14.1 ms; `/last` 7×–94× on wall time; the history route's four-way decomposition, including the 37.6-of-42.3 ms that is a no-op child. `doc/dev/cli/workflow.md` gains the commands that re-run each harness.
