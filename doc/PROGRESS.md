...
**Verification:** `cd cli && npm run test` — 87 files / 1370 tests green; `tsc --noEmit -p .` clean; golden matrix untouched (no command output changed). `scripts/verify.sh` passes every step except `app/server`'s `manifest.test.ts`, which fails **only in a worktree checkout**: `<worktree>/node_modules` is a symlink to the primary's, so `@yaco/codex-transcribe` realpaths to the primary branch's copy and esbuild reports it as `../../packages/…`, escaping the prefix the assertion expects. The same invocation at the same commit passes in the primary checkout, and it reproduces here at base `10e20e8e` — it is the `worktree-provision-wrong-cli` class, not this diff. Two cross-provider Codex review rounds.
