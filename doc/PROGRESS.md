...
**Verification:** `tsc -b` + `app/ui` lint green; `npm test` **1094 passed** (24 FilesPanel; one DialogShell-specific autofocus test removed). Isolated static-build e2e — the full worktree trio → **10 passed**. Cross-provider Codex review + `/qa` artifacts under `plan/all/worktree-explorer-view/`.
