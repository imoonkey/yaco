...
- **One missing workspace link closes `npm run start:app` AND `bash scripts/verify.sh` on a clean clone** — both die on `Cannot find package '@yaco/codex-transcribe'`. `installAppDeps()` runs `npm install` in `app/server` and `app/ui` but never at the workspace root, and the root install is the only thing that links `packages/*` into `node_modules/@yaco/`; `@yaco/codex-transcribe` is imported bare by `app/server/src/routes/voice.ts` and declared as a dependency by nobody. Proven in both directions — a root `npm install` creates the link and both commands then succeed. Unseen until now because every dev checkout has had a root install run in it once.
