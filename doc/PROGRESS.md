...
- **Rollback is documented as what to restore, with the verified revert beside it.** Replaying each recorded `git revert` at the milestone head was checked in a throwaway worktree: cutovers 2 and 4 now conflict, because later cutovers edited the same files. The page says so — a recorded SHA is evidence of what was proven, not a recipe to run today.
