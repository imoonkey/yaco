...
**Verification:** 40 focused live-runner tests and typecheck passed; focused
...
stream startups returned redacted `forbidden`; the current runner records that
non-transient reason once while completing the batch budget, so streaming latency/transcript
...
**What changed:**
- **`blockReason` had no way out.** The payload validator accepted only the five enum values, so `null` and `""` were both rejected and a task that had once been blocked carried its reason into every later state. `null` now clears it, spelled exactly the way `worktree` already spells an explicit clear — an absent key still means "leave it alone", so there is one convention, not two.
- **The field is now valid only alongside `state: "blocked"`, enforced on the write rather than refused at it.** The agent side has kept this identical invariant for the identical field all along (`setStatus`: *"`blockReason` is set iff status === `blocked`"*), so the task store was the outlier. Unblocking is one write and cannot leave the old reason behind. **Rejecting the pair was the alternative and it loses**, because it manufactures the bug it is meant to prevent: every caller that moves a task out of `blocked` and forgets the second write — `app/server`'s task routes, `/orchestrate`, a human at the CLI — produces exactly this state. Auto-clear turns the edge case into the canonical one.
- **But auto-clear never swallows an explicit request.** A reason named in the *same* payload as a non-blocked state is `INVALID`, not a silent drop; only a value this write did not mention is dropped quietly. Two enforcement points, both needed and neither redundant: after the payload merge, and inside the loop that runs after `deriveMilestoneStates` — a blocked leaf that gains a child becomes a milestone, and a milestone never settles on `blocked`.
- **`validate` reports a leftover pair; `doctor` does not fail on it.** `ValidationReport` gained `malformed`, true only for structural defects. `validate` fails on any non-empty bucket so a graph can be swept; `doctor` gates on `malformed`. That split is load-bearing: **`yaco install` throws on any failing doctor check**, and this repo's own graph carried six stale tasks, so routing the new check through `doctor` unfiltered would have stopped `yaco install` on this machine over a cosmetic field — with the repair path running through the same CLI.
- **Six tasks swept, not the one the task named.** Scanning every workset (71 files, 502 tasks) instead of trusting the count found four more sitting in the archive, blocked before they were archived. Auto-clear on write would never reach a task nobody rewrites, so the sweep is part of the change.

**Why:**
- `inline-suggestions-removal` was `state=ready` with `blockReason=human-review` and read as blocked in every surface that shows the field, while `yaco task validate` accepted the contradiction without a word.
- **Not derivable, so not repaired on read.** A milestone's state is recomputed on load and never reported; a block reason cannot be reconstructed, so clearing it at load time would make every reader disagree with the file. It is reported instead, which is what makes a one-time sweep possible.
- `set` is the only author of a leaf's `state` or of `blockReason`, so no command can mint a disagreement — **over a graph satisfying both that rule and `milestoneRollup`.** Both halves are needed, and the qualification came from review: a milestone recorded `blocked` on disk agrees with itself on the pair, but the derivation moves it and a whole-store save (`archive`, `rm`) then persists the non-blocked state beside the surviving reason. `attach`/`detach` carry a legacy pair rather than repairing it, deliberately — `link.ts` already declines, in those words, to persist normalization of fields it was not asked about.

**Key files:** `cli/src/commands/task/{set,validate}.ts` · `cli/src/commands/doctor.ts` · `cli/src/lib/core/task/{graph,validation}.ts` · `cli/test/{integration/task/task-cli.integration.ts,unit/core/task/{graph,validation}.test.ts,unit/commands/doctor.test.ts}` · `doc/main/cli/task.md` · `agent-config/global/skills/yaco-task/SKILL.md` · `plan/all/task-blockreason-clearable/{plan,qa,review}-*.md`
**Verification:** `scripts/verify.sh` green. CLI unit suite re-run in a hand-built provider-free `$PATH` with `claude -> ABSENT` / `codex -> ABSENT` asserted in the same output (92 files / 1484 tests) — CI has no provider on `$PATH` and this desktop does. `task-cli.integration.ts` 43 passed (9 new); `app/server` routes 220 passed. Every new test mutation-checked, five mutants written and all killed, including the unconditional-clear one the reviewer named. Sweep rehearsed against a copy of the real graph before touching it: 4 files, 6 reasons removed, 502 tasks before and after, then `validate` exit 0, `doctor` `task-graph: pass` / `summary.fail: 0`, and a raw re-scan reporting zero disagreements. Cross-provider Codex review, 4 rounds, **APPROVE**, 0 unresolved Critical/High.
**Commit:** `1108cfe5`..`1947135c` (+ `plan` repo `c48891a` for the sweep)
**Next:** `validateGraph` enum-checks `state` and `workset` but not `priority`, `estimate`, or `blockReason` — worth one decision covering all three, including whether an invalid value should be able to fail `doctor`. Separately, `dependency` is a legal `blockReason` while the convention is that dependency-waiting tasks stay `ready` and `blocked` is only for non-dependency blockers; the enum and the convention contradict each other. Both recorded, neither changed here.
**Blockers:** None
...
## 2026-08-12: a project is a subtree in history too, not just in the live list

**What changed:**
- **The two halves of "what has run in this project" answered with two different scoping rules, and history was the odd one out.** The live session list has always matched a *subtree* — `listByPath`'s prefix match, `resolveProjectForPath` / `isPathDescendantOrEqual` — while both history readers keyed on an exact cwd: Claude opened the single `~/.claude/projects/<encoded-cwd>` directory, Codex queried `WHERE cwd = ?`. So every `/orchestrate` worker and reviewer, which runs in `<project>/.worktrees/<slug>`, was listed while it ran and vanished the moment it was only history. Both readers now apply the live list's predicate. On this machine the project window goes from **5 branches and no `task/*` at all to 36 distinct worktree branches**.
- **The Claude half is per *log*, not per directory, and that was the review's doing.** The first implementation found candidate directories by encoded-name prefix and confirmed each with the first `cwd` one of its logs recorded. `encodeClaudeCwd` collapses every non-alphanumeric to `-`, so that is unsound in the exact case it defends against: `/repo/demo/a_b` and `/repo/demo-a-b` land in one directory, and so do `/repo/demo` and `/repo:demo`. The cross-provider reviewer built fixtures showing valid history disappearing in one `readdir` order and unrelated history leaking in the other. Attribution moved into `claudeTail`, which takes the cwd out of the tail slice it reads anyway — **exact, order-independent, and no extra I/O**. Two more rounds sharpened it: a log recording no cwd belongs to **nothing** (falling back to the directory it was filed under kept the very leak per-log attribution removes, since the project's own encoded name is just as lossy), and the cwd is matched in the **raw bytes** rather than in a parsed record, because a single record can be larger than the slice — a log whose first record is 20 KB and last is 70 KB has no whole line in either window, and the reference home already holds 219 records over 64 KB. Taking the last match is what makes byte-matching safe: escaped text in a prompt cannot match at all, a nested `cwd` key can but lives inside `message`, and Claude writes `message` before the record's own field. Byte-matching agrees with a whole-file parse on all 1 053 logs of the reference home, and with the parsed field on 157 523 of 157 523 cwd-bearing records. `cwd` is the **only** field read that way, and round 5 is why: a byte match is sound only for a field nothing content-bearing follows, and a timestamp fails that on both sides — `toolUseResult` (a tool's raw output, arbitrary JSON) is serialized between a record's timestamp and its cwd on 48 075 records, and `message` precedes the timestamp on all but 106 of 50 000 sampled. So a log whose last record exceeds the tail is still ordered by its file mtime, which can cost it a place in the window; that is now a stated limit with a task against it (`history-oversized-record-order`) rather than a fix built on an anchor that does not hold. Reading further would not settle it either — records reach 1.3 MB and 48 exceed 512 KB.
- **Codex matches the subtree with `substr(cwd, 1, length(?)) = ?` against the literal `<cwd>/`, not `LIKE`.** A path is not a pattern: `%` and `_` are legal in paths and are wildcards in `LIKE`, which also folds ASCII case where a POSIX path does not. The prefix carries exactly one separator — the filesystem root already ends in its own, and appending a second matched no absolute path at all, which is how a project at `/` returned nothing below it in both providers.
- **Widening the query is a re-judgement of a measured admission, not a test to update.** It moves off `idx_threads_archived_cwd_recency_at_ms` onto a plain `archived` scan, so `RULE_5_SQLITE`'s prepared statement, emitted JavaScript and stall bound were re-pinned from a fresh probe: 8.6–9.1 ms p50 on the busiest subtree (867 rows) against 3.0 ms on the busiest single cwd. The bound that governs is still the design's own, and it was re-run twice on the real home — in-process p95 timer starvation 18.1 ms against the subprocess route's 30.5, and 23.1 against 28.2.
- **The cap is taken once, over the union.** A per-directory cap would let one busy worktree crowd the project's own sessions out and the window would stop being a prefix of the merged order. A thread resumed under a second cwd is now reachable twice, so tails are deduplicated newest-first with the log path breaking a tie.
- **The cap fixture proved less than it claimed**, also caught in review: with every session id unique across directories, a cap taken per directory before the union recovers the same window, so its deep-equals passed either way. The newest six Claude sessions are now logged under all three cwds seconds apart, and a cap taken before the dedup fails four tests. Every fix in this task was mutation-checked the same way — the mutant is written, the suite is run, the specific test is confirmed red.

**Why:**
- The user noticed worker sessions had stopped appearing in history and asked whether including them was even feasible without slowing the list. It is: the row fan-out does not grow at all, because the cap is applied after the union, and the added work is 39 extra tail reads and one `readdir`. The deeper reason to do it is that the inconsistency was arbitrary — nothing ever decided history should scope differently from the list beside it.

**Key files:** `cli/src/lib/core/agent/providers/history.ts`, `cli/src/lib/core/agent/providers/codex-thread-window.ts`, `cli/test/history.test.ts`, `cli/test/unit/export-audit.test.ts`, `cli/test/fixtures/rule5-sqlite/codex-thread-window.emit.js`, `cli/test/bench/history-stall.ts`, `doc/main/cli/{read-path,exports,architecture}.md`, `doc/main/app/ui/workspace/sessions-and-terminal.md`
**Verification:** `scripts/verify.sh` green; CLI unit suite 1 419 tests. Seven cross-provider Codex review rounds, final **APPROVE, 0 Critical / 0 High** at the shipped sha (`plan/all/history-subtree-scope/review_codex.md`); five of the findings were real defects in code written for this task, two of them in fixes made for earlier findings. End-to-end QA of both affected flows against the operator's real provider homes, with the before/after contrast measured against the installed build: `plan/all/history-subtree-scope/qa.md`.
**Commit:** `686b7de9..3ab06bd2`
**Blockers:** None.

## 2026-08-12: `agent start` says which provider is missing, and repeats what the provider said

**What changed:**
- **A precheck, for the one failure yaco can name in advance.** `start` creates a tmux session for a provider executable that may not exist; `doctor` and `agent status` both already probed `which(<executable>)` and both reported `not found` on the same machine. Only the start path skipped it. It now refuses with `CliError(ENV)` (exit 3) naming the executable — thrown above `writeState` and above `createSession`, so a refusal has nothing to orphan. `src/lib/core/which.ts` is that probe, once: `doctor`, `agent status` and `agent start` had three private copies of the same question and must not be able to disagree about whether `claude` is installed.
- **The check sits in `runStart`, not only in `start()`** — found by review. `--wait --resume` resolves a cursor out of the provider's own logs *before* launching, so a `start()`-only guard let `yaco codex resume <id> --wait` fail as `cannot resolve resume cursor`: true, and not the reason.
- **A channel for every other bootstrap failure, which is the wider defect.** `start` binds four yaco flags and forwards every other token to the provider CLI verbatim — deliberately, since that is what lets a caller pass arbitrary provider arguments through. The consequence is permanent: a mistyped yaco flag *is* a provider error, and yaco cannot validate it away without taking passthrough with it. So the fix is carriage, not validation. On the same non-zero-exit branch that already writes the crash tombstone, `agent-wrapper.sh` captures its own pane into `${sessions}/.exit-<handle>` (createdAt as generation guard, exit code, up to 40 lines); `start` reads it before `deleteState` and appends it to the message. With no report the sentence is byte-identical to the old one.
- **The wrapper is the only thing that can do it, and being inside the pane is not enough.** The EXIT trap runs while the pane still exists — the wrapper *is* its process — which is why this stayed shell. But the pane's screen is not the pty: tmux fills the screen from an asynchronous read while `capture-pane` reads the screen, so a report could be well-formed, correctly generation-stamped, race-free for its reader, and still missing the diagnostic it exists to carry. Closed by emitting a marker after the provider's last byte and waiting for it to come back — the read is FIFO over one stream, so the marker cannot appear until everything ahead of it has been parsed in.
- On the real Claude CLI, `yaco claude --nmae foo` had been printing `(Did you mean --name?)` all along, into a pane destroyed before anyone could read it. It now reaches the caller.

**Why:**
- The first agent command a stranger runs is `yaco claude "…"`, and on a machine without it that returned `Session "claude-…" died during bootstrap` — never naming `claude`, while `doctor` on the same machine named it. This is the contract of the `doctor-providers-stranger` change applied one command later: install tolerates an absent provider *and keeps the condition visible*, which is hollow if the next command still dies anonymously.
- The lost-stderr half is the more general one and a precheck cannot cover it, because the class it belongs to is unbounded — any argument any provider ever rejects, plus broken installs, plus failures nobody has thought of. The evidence for that came unplanned: two integration tests that already failed at `d12ba571` for their own reasons now report `codex exited 1: … no rollout found for thread id …` instead of the anonymous line.
- The barrier is kept despite 40/40 trials failing to reproduce the race it closes (two shapes — a 250KB burst and a single line before an immediate exit — the tail arrived every time without it, and the marker returned on the first capture every time with it). The ordering hole is established from tmux's structure; 40 samples on one idle machine do not close it, and the cost is one `capture-pane` on a path that has already failed. The reviewer was asked directly and agreed.

**Key files:** `cli/src/lib/core/which.ts` (new) · `cli/src/commands/agent/{start,index,status}.ts` · `cli/src/commands/doctor.ts` · `cli/src/lib/core/agent/session-state.ts` · `cli/scripts/agent-wrapper.sh` · `cli/test/{exit-report.test.ts,unit/commands/agent/start-precheck.test.ts,integration/crash-contract.integration.ts}` · `doc/main/cli/{lifecycle,providers,doctor,state-contract}.md`
**Verification:** `scripts/verify.sh` green; cli typecheck + 90 unit files / 1447 tests; golden matrix byte-identical; `crash-contract.integration.ts` 6/6 including the two new wrapper cases; a six-case before/after start matrix (no provider · fake provider rejecting a flag · fake provider healthy · **real** claude rejecting a flag), each in its own `HOME`/`YACO_HOME`/tmux server, checking output, exit code, leftover session state and leftover tmux on every case. Cross-provider Codex review, 2 rounds, APPROVE, 0 unresolved Critical/High.
**Commit:** `aa74e179`..`4845a30f`
**Next:** `agent status` could surface a crashed tombstone's exit report — the file outlives the tombstone and nothing reads it today. Deliberately not built here.
**Blockers:** None

## 2026-08-12: `yaco install` links `packages/*` — the clean-clone install was broken

**What changed:**
- `installAppDeps()` ran `npm install` in `app/server` and `app/ui` and **never at the workspace root**, and the root install is the only thing that links `packages/*` into `node_modules`. `app/server/src/routes/voice.ts` imports `yaco-codex-transcribe` **bare**, declared by nobody, so on a clean clone `npm run start:app`, `bash scripts/verify.sh` **and** `npm pack --workspace yaco-app` all died on it. Now one `npm install` at `repoRoot`, which covers both app members (they are workspace members) and does the linking.
- **The tempting fix is wrong and is recorded as such** in a comment at the fix, in `install.md`, and in a test name: `app/server/scripts/build.mjs` externalises exactly the *declared* dependencies and inlines this one precisely because it is not declared, so declaring it would make the published bundle import a package that is never published (`private: true`). Proved still true on the packed head: 283 KB bundle, no external import of any undeclared workspace package, all three transcribe markers physically present.
- **The step now asks who owns the `node_modules` before writing to it.** A linked worktree's is not its own — `scripts/worktree-provision.sh` builds it as a mirror in which every third-party package and `.package-lock.json` itself is a symlink into the main checkout (468 of them in the worktree this was written in), so `npm install` there would rewrite the main checkout's tree from the worktree's branch. `nodeModulesOwner()` answers `self` | `linked-worktree` | `unknown` and only `self` proceeds; it reads git's topology (`--git-dir` vs `--git-common-dir`), not the filesystem, because a submodule and a `--separate-git-dir` checkout carry the same `gitdir:` file while owning their tree outright. Any answer git cannot give — no git on `$PATH`, damaged metadata, output that is not exactly two non-empty lines — is `unknown` and skips.
- The `--cli-only` guard moved from directory shape (`app/server` exists) to repository identity (`isYacoCheckout`), which is what `cli/CLAUDE.md` already claimed it was.
- Unrelated, at the operator's request: `yaco agent --help` listed `start` as `[yaco-flags]` and named none of them, so `-n`/`--name` was findable only by reading the parser. It now names the four flags `start` binds and states that every other token is forwarded verbatim to the provider CLI — which is why a mistyped yaco flag reaches the provider, and a provider that rejects it exits before yaco can say anything better than "died during bootstrap".

**Why:**
- It does not block an npm publish — the maintainer's tree has had a root install, and the tarball a `npm install -g yaco-cli` user gets carries the code inlined. It blocks **making the repository public**, which is the moment the README's `git clone` instructions start applying to real people.
- **A correction to the correction.** The worker brief for this task said `npm pack --workspace yaco-app` succeeds on a clean clone and only `start:app`/`verify.sh` were affected. That measurement had been taken in a tree that already had a root install. On a genuinely virgin clone the pack fails too — `prepack` runs the server build and esbuild cannot *resolve* the undeclared package in order to inline it. The task record's original framing was right. Every measurement in the QA artifact states, in its own log, whether the tree had been repaired.
- No new detector was written: `app/server/test/workspace-resolution.test.ts` already reads the workspace specifiers off `app/server`'s source and fails `verify.sh` at `server test` when the linking is missing. The base arm's log shows it firing.

**Key files:** `cli/src/commands/install.ts`, `cli/test/unit/commands/workspace-root-install.test.ts` (new), `cli/test/unit/commands/install.test.ts`, `cli/src/commands/agent/index.ts`, `cli/test/agent-dispatch.test.ts`, `cli/test/golden/{matrix.json,ordering-delta.test.ts}`, `doc/main/cli/install.md`, `cli/CLAUDE.md`, `doc/dev/{cli,app}/workflow.md`
**Verification:** `scripts/verify.sh` green on the head — and green **inside a virgin clone**, which is the run that counts. Two-arm hermetic sandbox (a whole clone and a whole `HOME` per arm, `PATH` built from hand-placed symlinks, `env -i`): base `4ca22e0c` fails `start:app` (`Cannot find package`), `verify.sh` (at `server test`) and `npm pack` (esbuild `Could not resolve`); head `706c2328` passes all three plus the bundle-inlines check. The install phase asserts a virgin tree and fails closed — it voided one run when a re-clone left the previous `HOME` in place. Five rounds of cross-provider Codex review, final **APPROVE, 0 Critical / 0 High**; four of the five findings were real defects in code written for this task. Nothing published; `~/.local/bin/yaco` and the server on 3001 untouched.
**Commit:** `8d721917..706c2328`
**Next:** the remaining release blockers are the private repository and the unpublished packages (`yaco-cli` must publish first).
**Blockers:** None.

## 2026-08-12: the published packages are unscoped — `yaco-cli` and `yaco-app`

**What changed:**
- The npm organization `@yaco` **cannot be registered** — npmjs rejects it as unavailable, almost certainly under its too-similar-to-an-existing-name rule, since an unrelated 2022 jQuery plugin owns the unscoped name `yaco`. Both `yaco-cli` and `yaco-app` were confirmed free, and were chosen over a personal scope because `npm install -g yaco-cli` is the line a stranger reads and it needs no org at all.
- `@yaco/cli` → `yaco-cli`, `@yaco/app` → `yaco-app`, `@yaco/codex-transcribe` → `yaco-codex-transcribe`. The last one is never published (`private: true`) and moved anyway: one scoped name left behind keeps the `@yaco` scope directory alive in `node_modules`, and a stale import inside it goes on resolving instead of failing.
- **Two call sites named the package where they meant the directory, and both stopped.** `tools/install.sh` packs `--workspace cli`, so the bootstrap no longer has to be edited when the published name changes. `app/server/test/workspace-resolution.test.ts` selected the specifiers it audits with a `@yaco\/` regex; it now derives them from the root manifest's `workspaces` globs — the same source of truth `scripts/worktree-provision.sh` reads. A bare name carries no shape that separates a first-party workspace from a registry package, so *deriving* the set is the only honest selection, and a package added, removed or renamed needs no edit there again.
- **The one-segment branch of `worktree-provision.sh` had never executed.** It accepts a workspace name as one segment **or** `@scope/name` and mirrors accordingly, but every fixture in its hermetic suite was `@fx/*` — and one segment is now the shape this repo takes. A bare self-link sits in the *same directory* as every third-party package, so depth no longer separates them and only symlink-vs-real does. §19 pins it: an unbuilt `exports` subpath resolves to the worktree's own copy, a `.bin` shim written through the bare name re-anchors into the worktree, and a third-party package at that same depth still comes from the main checkout. Shown non-vacuous — with the mirror's verbatim symlink-target copy mutated away, the new `.bin` assertion goes red alongside the scoped one.
- `publishConfig.access: "public"` was **dropped** from both published manifests. It was added the day before because a *scoped* package publishes restricted unless told otherwise; unscoped is public by default, so the line no longer states anything. Publishing under a scope again would fail loudly at the registry without consuming a version, so the removal's failure mode is free.
- `doc/PROGRESS.md` and `doc/progress/**` keep the old names on purpose — they record what was true when they were written.

**Why:**
- The rename is forced, but the two derived-from-the-directory changes are not: they are what makes the *next* rename a one-line diff instead of an 88-file one. Every place that still spells a package name now does so because the name is genuinely what it means.

**Key files:** `cli/package.json`, `app/server/package.json`, `packages/codex-transcribe/package.json`, `package-lock.json`, `tools/install.sh`, `scripts/verify.sh`, `scripts/worktree-provision.{sh,test.sh}`, `app/server/test/workspace-resolution.test.ts`, `cli/test/helpers/export-closure.ts`, `README.md`, `doc/main/**`, `doc/dev/**`
**Verification:** `scripts/verify.sh` green end to end, CLI pack smoke included, and the golden output matrix byte-identical (it keys on export *subpaths*, which carry no package name). `worktree-provision.test.sh` 78 assertions. `npm pack --dry-run` on both packages: right names, right file lists, nothing published. Resolution proved by **running Node, not by reading a symlink**, in a throwaway clone: `npm ci --ignore-scripts` exit 0 (so the regenerated lockfile is in sync), no `@yaco` scope directory built, `yaco-cli/core/*` resolves inside the clone and `@yaco/cli` no longer resolves at all. Then a git worktree off that clone, provisioned and unbuilt: `import('yaco-cli/core/task')` fails `ERR_MODULE_NOT_FOUND` on a path physically inside *that worktree*, not in the clone it was made from.
**Commit:** `90f57dbc..HEAD`
**Next:** publication is the operator's step and needs their credentials — `npm pack --dry-run` is as far as this went. **Deploy-day step, reproduced in an isolated prefix:** the old and new packages are different packages competing for the same `bin` name, so on a machine that already has the scoped globals the first install of the new ones — including `bash tools/install.sh` — dies with `npm error code EEXIST` on `<prefix>/bin/yaco` and installs nothing. `npm uninstall -g @yaco/cli @yaco/app` first; the install is then clean and the CLI runs. This is deliberately *not* wired into `tools/install.sh`: it is a one-time migration, and a permanent uninstall-the-old-scope line would be dead from the second run onward.
**Blockers:** None.

## 2026-08-12: the npm install path, walked once on a machine that had never seen YACO

**What changed:**
- **The install path works, and this is the first evidence of it.** `cli-node-sdk` replaced the Bun binary with an npm package on 2026-08-11, and both installs since were upgrades over the old binary — both hit `npm error EEXIST`, a state no clean machine can be in. A hand-built sandbox (empty `HOME`, a `PATH` assembled from three hand-placed symlinks plus `/usr/bin`, its own install prefix) ran `tools/install.sh` verbatim: **exit 0 in 32 seconds**, CLI on `PATH`, 22 skills linked, both provider hooks merged, repo registered, doctor **9 pass / 0 fail**. `bash scripts/verify.sh` in that same clone is green in 1m51s — but only *after* the workspace-root repair below; on the clone as `tools/install.sh` leaves it, it fails.
...
- **One missing workspace link closes three things on a clean clone, including the release artifact.** `npm run start:app`, `bash scripts/verify.sh`, and `npm pack --workspace @yaco/app` all die on `Cannot find package '@yaco/codex-transcribe'` — so the app package cannot be *built* from a clean clone, which makes this a release blocker rather than a first-run annoyance. `installAppDeps()` runs `npm install` in `app/server` and `app/ui` but never at the workspace root, and the root install is the only thing that links `packages/*` into `node_modules/@yaco/`; the package is imported bare by `app/server/src/routes/voice.ts` and declared as a dependency by nobody. Proven in both directions — a root `npm install` creates the link and all three then succeed. Unseen until now because every dev checkout has had a root install run in it once. **The tempting fix is wrong**: declaring it as an app dependency would break the published package, because `app/server/scripts/build.mjs` externalises exactly the declared dependencies and inlines this one *by not declaring it*. The fix is the workspace-root install.
- **The existing gate would have caught it, if it had ever been pointed at a clean clone.** `app/server/test/workspace-resolution.test.ts` — written for the worktree-provisioning bug — reads the `@yaco/*` specifiers off `app/server`'s own source and asserts each resolves inside the checkout, so it names this exact package; `verify.sh` fails at `server test`. The first draft of this report claimed the opposite, reasoning from a `verify.sh` run that had unknowingly been made *after* a repairing root install. The cross-provider reviewer challenged that claim and reproduced the failure from a `git archive` of the reviewed sha; it was then re-run in the sandbox with only the link removed. Ordering is now stated in the report so no later reader has to re-derive which runs were clean-state.
...
- **The app package itself is sound, and its `bin` wiring is real.** Naming both tarballs in one `npm install --global` satisfies the app's CLI dependency from the tarball beside it (exit 0, 256 packages) and npm writes both launchers into the prefix; `yaco-app` then runs off `PATH` from a directory with no checkout above it — `YACO server running`, `/api/health` 200, `/` 200 — and `yaco` from the same prefix works too. Its entry point is that `yaco-app` binary, which the README had never named while telling npm-only readers to start layer 3 with a repo script — fixed where it does not collide with the rename worker's lines.
...
- **Cross-provider review found five defects in the first version of the fix, three of them in the safety story it was claiming.** (1) The main-checkout guard compared git's *physical* path against the *logical* `$PWD`, so reaching the primary checkout through a symlinked alias did not skip — it mirrored the main install onto itself, unlinking as it went. Both sides are canonicalized now and `mirror` refuses a source and destination that resolve to the same directory. (2) Workspace names and directories came from a manifest and went straight into paths the script writes with; a name of `../../node_modules/sentinel` replaced a real file in the main checkout before the audit ran. Both derived destinations are now proved to be inside the worktree before the first write, and the workspace loop unlinks only symlinks — which is what the header comment had been claiming all along. (3) The mirror only ever added, so a dependency removed from main survived in every worktree; it now drops links to entries the source no longer has. (4) Records were space-delimited, so a workspace path with a space became two fields — tab-delimited now, and the self-check compares *paths* rather than the resolver's percent-encoded URLs, which had the same failure for the same reason. (5) The test's `fingerprint` used GNU-only `find -printf`: under a BSD `find` it returned the empty string, and the two assertions proving the main tree was unchanged and the second run idempotent compared empty to empty and reported themselves as passing. It is Node now, and a fingerprint that cannot be taken aborts the run. Round 2 then found that the containment proof did not survive its own transport: a package name of `safe<TAB>../victim` passes both checks as one string and re-splits in the shell into name `safe` and directory `../victim`, which wrote a tree outside the worktree. A NUL is the same class one byte over — command substitution *drops* it, so `safe/..<NUL>/../../victim` is inside `node_modules` while Node holds it and one level above the worktree once the shell has it. Any control character in a record is now refused rather than rewritten, so what reaches a write is exactly what was validated. Both cases fail against the unguarded script for the right reason — a tree, and a link, created outside the worktree — not merely on the exit code.
- **A second independent review of the corrected script found three more, one of them the same shape once again.** (1) Lexical containment is not containment: a workspace named `leftpad/lib/local` resolves under this worktree's `node_modules`, but by the time the write happens `leftpad` is a link to *main's* copy of that dependency, so `mkdir`/`ln` follow it and leave a symlink inside the main install — reproduced, and now a fixture that fingerprints main across the attempt. Names are held to the only two shapes npm can publish (one segment, or `@scope/name`), and the destination is canonicalized at the write seam; each guard alone is sufficient, and the fixture is red only when both are removed. (2) The self-check probed the first `./` key in `exports`, which npm allows to be a **null** target blocking an internal path — so a correctly provisioned worktree failed creation on nothing but key order. It now picks the first subpath with a usable target. (3) Convergence held for an entry inside a mirrored directory but not for a mirrored *directory*: if main lost `.bin` or a workspace's whole nested `node_modules`, the worktree kept it, and a stale `.bin` shim whose package still exists stays runnable. Both are pruned now, with `rmdir` rather than a recursive delete, so a worktree-owned entry still keeps its directory.
- **The round after that took the last piece of prediction out of the self-check.** Skipping a null `exports` target was still guessing: a subpath can also be blocked under the condition this resolver actually applies (`{"types": "./t.d.ts", "default": null}` reads as usable and resolves to nothing), so a correctly provisioned package failed creation on key order once more. The script no longer decides which subpath is resolvable — it offers the declared subpaths in order and keeps the first the resolver answers, which is the same move as reading the resolver for the checkout question instead of reading the symlink. The write-seam guard also moved ahead of the `mkdir` it was supposed to guard, and canonicalizes the deepest *existing* ancestor, so a name whose intermediate directory does not exist yet is stopped before `mkdir -p` walks a dependency link into main. The now-unreachable lexical name check was deleted rather than kept as a third layer. 73 assertions, and each guard was run against a variant of the script with that guard removed: name-shape alone and write-seam alone each still refuse the multi-segment name, and only removing both leaves a link inside main for the fingerprint to catch.
...
**Verification:** `cd cli && npm run test` — 87 files / 1370 tests green; `tsc --noEmit -p .` clean; golden matrix untouched (no command output changed). `scripts/verify.sh` passes every step except `app/server`'s `manifest.test.ts`, which fails **only in this worktree**: its `app/server/node_modules` is a symlink to the primary checkout's, so `@yaco/codex-transcribe` resolves out of the primary tree and esbuild's metafile reports it as `../../packages/…`, escaping the prefix the assertion expects. The same invocation at the same commit passes in the primary checkout and in all four sibling worktrees (which have their own `app/server/node_modules`), and it reproduces here at base `10e20e8e` — it is the `worktree-provision-wrong-cli` class, not this diff. Two cross-provider Codex review rounds.
...
**What changed:**
- `app/server`'s History tab calls `@yaco/cli/core/agent`'s `readProjectHistory` in its own process; `yaco agent history` is now the argv-and-render adapter over the same function. `fetchHistory` and its DTOs are deleted — the rollback target is the CLI command plus the call site named in `read-path.md`, not a dead second reader of the same data.
- **The reader had to be rewritten to pass, and it is the rewrite that is interesting.** Two decisions the predecessor's benchmark could not answer: the per-provider cap is `limit + 1` and `--since` needs no plumbing at all, because the merge and the cutoff key on the same `updatedAt`, so the rows past any cutoff are a prefix of each provider's newest-first order (`+1` keeps `truncated = matching.length > limit` exact, which a cap of `limit` would report as a full untruncated window); and a provider's cap key **must be the key the merge sorts by**, which retired the prototype's Claude cap on `stat` mtime — a Claude row's `updatedAt` is the index's `modified`, else the log's own last timestamp. Claude is read in two phases instead: tail-read every log for the ordering key, head-read only the window.
- Everything the prototype dropped is restored and inside the measurement: `sessions-index.json` enrichment, the Claude first-user-message summary, the Codex thread-name index, sidechain filtering, live tagging. It is affordable because three parsers now scan backward and stop at the first hit — the last match found scanning backward *is* the last match — so a 64 KB tail costs a `JSON.parse` rather than a scan.
- `origin.ts` split: `origin-read.ts` is the async chunked leaf the read reaches, `origin.ts` keeps the writer and imports from it, so an exported closure that needs origins contains no origins writer. The per-row `readFileSync` (up to 200 a request) is gone.
- **Five instrument corrections, every one of which had been flattering the result, and three of them found by review.** (1) The fixture's composite index was named `…_updated_at_ms` but indexed `updated_at`, so the benchmark measured an index-prefix scan where production sorts in a temp B-tree. (2) The fixture wrote 200 Codex name-index records at *both* scales while the reader splits and scans that whole file — a tenth of the reference home at 1x, a hundredth of what 10x claims. (3) Fixed to the right record count, it was still 55% of the measured *bytes*, and its ids were all unique so last-entry-wins was never exercised; the count and the byte total are both pinned now, hit exactly, with ~11% repeated ids. (4) The verdict was computed over every in-process form, so it would have printed success whenever any control cleared the bound, including a run where the shipped route did not. (5) The harness's own header told the reader to quote the bound from a full run — the one that inflates the subprocess side alone.
- **The control was on the wrong axis, and measuring it is what showed that.** A control that removed only the cap *starves less* than what ships — a bigger scan yields more often. The chunked yield bounds the stall; the cap bounds wall time, and at ten times scale bounds both. The control is now the retired reader checked in verbatim (`history-retired-control.ts`), which is over the bound on all three fixtures while the shipped reader is within on all three.
- Fixture construction moved into a child process, which removes the standing reviewer caveat that the 10x spawn baseline carried the benchmark's own heap. A second heap confound remains and is measured rather than waved at: the harness's own routes grow one, `spawn-noop` p95 is 99.3 ms in a full 10x run and 33.2 ms under `--routes spawn-noop,subprocess,in-process`, and the header says which run to quote which figure from.
- Rule 5 judges a second `node:sqlite` query, `codex-thread-window.ts` — alone in its own module, because the admission pins the JavaScript the module compiles to and one module carrying two measured queries would make every edit a re-judgement of both. `RULE_5_SQLITE` gains a per-admission `bench` field; `RULE_5_DEBT` is still empty.
- Out of the review: `runYacoAgentJson` built its structured `yaco <cmd> failed [CODE]: msg` error *inside* the same `try` whose `catch` absorbs a non-JSON stderr tail, so it caught the throw as well — the translation had never once run and every subprocess route surfaced the opaque `exit <code>: <stderr>`. Fixed, with three tests that fail against the old shape.

**Why:**
- The predecessor task measured rather than assumed, found the shipped reader over the bound, and stopped at the gate rather than widening into four files another task was being held back for. This is the successor it specified.
- The read *should* have moved: 37.6 of the retired subprocess route's 42.3 ms p95 was a no-op child. "Keep the subprocess" was never the safe side of that comparison.

**Measured** (p95 worst starvation of an already-queued timer, before → after; route wall p50 in brackets). Taken from `--routes spawn-noop,subprocess,in-process`, because review found the full run inflates the *subprocess* side alone — a fork inherits the parent heap the controls grew, an in-process call pays no fork — which was worth ~80 ms of false headroom at 10x:
- real provider home (11.1 MB `state_5.sqlite`, 2 301 threads, 81 Claude logs) — **26.8 → 12.4 ms** (206 → 119 ms)
- synthetic 1x — **29.7 → 13.7 ms** (154 → 83 ms)
- synthetic 10x — **29.8 → 18.4 ms** (285 → 260 ms)
- the control, from the full runs where separation rather than a bound is the question: retired **64.5 / 66.7 / 345.1 ms** against the shipped reader's 11.4 / 13.9 / 31.7 on the same three
- the judged query alone: 3.0 ms p50, 5.4 p95, 8.3 max over 40 warm samples, plan printed

**Key files:** `cli/src/lib/core/agent/{index,origin,origin-read}.ts`, `cli/src/lib/core/agent/providers/{history,codex-thread-window,types,claude,codex}.ts`, `cli/src/commands/agent/history.ts`, `app/server/src/lib/{history,agent}.ts`, `cli/test/{history,agent-history-parity}.test.ts`, `cli/test/unit/{export-audit,core/agent/{origin,ordering}}.test.ts`, `cli/test/bench/{history-stall,history-fixture,history-retired-control,history-entry}.ts`, `cli/test/fixtures/rule5-sqlite/codex-thread-window.emit.js`, `app/server/src/lib/__tests__/{history,agent-failure-envelope}.test.ts`, `doc/main/cli/{read-path,exports}.md`, `doc/main/app/backend/libs.md`, `doc/main/app/ui/workspace/sessions-and-terminal.md`, `doc/dev/cli/workflow.md`
**Verification:** `scripts/verify.sh` (one pre-existing failure, `app/server` `manifest.test.ts`, confirmed at the base commit by stash-test); CLI `tsc --noEmit -p .` clean; golden matrix byte-identical; export audit green with `RULE_5_DEBT` empty; two cross-provider Codex review rounds.
**Commit:** `825cb134..HEAD`
**Next:** none in this milestone — Phase 2's five cutovers are landed.
**Blockers:** None.
...
- The golden matrix was recaptured for the one record that moved — this task's own delta is that single record, `fail` → `skip`, and the case summary with it (8 fails → 7). `INTENTIONAL_DELTAS` now names `providers` alongside `registry` and `skills-link`, and `ASSERTED_TRANSFORMATIONS` pins the record rather than waiving it: both `providers` records asserted whole with `toEqual`, exact detail strings included. Note that what those assertions compare is the *historical* pair — `matrix.original.json` against `matrix.json` — so the summary they pin is `{pass 2, fail 9}` → `{pass 3, fail 7}`, which carries this task's change plus the earlier ones already declared in that entry. The other eight check records in that case go on being compared.
...
**Verification:** `bash scripts/verify.sh` passes in full (1346 CLI unit tests, the 13-case pack smoke, and the `app/server` suite). It first died at the `app/server` step for a reason unrelated to this diff — a worktree's `node_modules` is symlinked at the main checkout's, whose `@yaco/cli` lacks two exports this branch's `app/server` imports, `ERR_PACKAGE_PATH_NOT_EXPORTED`. Confirmed pre-existing by stashing the whole diff and re-running at the base commit: identical 10 files / 26 tests. Repaired with the worktree-local `app/server/node_modules/@yaco` symlinks recorded under standing task `worktree-provision-wrong-cli`, which leaves the primary checkout untouched and adds nothing to the diff. Separately, a 34-assertion hermetic QA harness drove the real tarball end to end: install exits 0 with no provider on `$PATH`, the skip is visible in text and JSON, a provider appearing flips it back to pass, and deleting the wrapper or the skill links still fails doctor and install — the change did not make the closing doctor a rubber stamp. See `plan/all/cli-node-sdk/{qa,review_codex}-doctor-providers-stranger.md`.
**Commit:** `c530bbeb`
...
**What changed:**
- New `doc/main/cli/read-path.md`. Each landed cutover documented itself well, but the milestone-level answer — which routes moved, which did not, what each move measured, and how to put one back — was spread across four `plan/` artifacts and a design table. It is now one page: the three conditions that admit a read into `app/server`'s process, a measured before/after row per cutover, the accepted limits, what still spawns and why, and a rollback statement per cutover.
- **The rule is stated as the conjunction it is.** Export eligibility is necessary and not sufficient: a path also has to be a read rather than a mutation, and it has to measure a starvation bound against the complete subprocess route it replaces. The history read is the case that separates the two — it was measured, the shipped reader failed the bound, and it is therefore not exported at all. `doc/main/cli/exports.md` now says so at the top, so passing the audit is not read as permission to move a route. Where a landed cutover does *not* meet the starvation condition everywhere — cutover 1 on a multi-megabyte single-file store, cutover 4 at the corpus extreme — the page records the exception rather than restating the rule around it.
- **The measured figures are in `doc/`, not only in `plan/`.** Task GET 181.6 → 28.5 ms and the read alone 427.3 → 72.1 ms at 4 850 tasks; summaries 122 → 30 ms real home and 138 → 36 ms at ten times, starvation 27.1 → 13.4 and 34.3 → 14.1 ms; `/last` 6.4×–94× on wall time; the history route's four-way decomposition, including the 37.6-of-42.3 ms that is a no-op child. The provider catalog had no recorded route figure — the milestone measured its *absence of I/O*, not a speedup — so one was taken for this page and labelled as such: 72.7 ms per spawn against 0.0003 ms in process. `doc/dev/cli/workflow.md` gains the commands that re-run each harness.
- **Rollback is documented as the call site to restore, with a replay matrix beside it.** Every recorded `git revert` was replayed in a throwaway worktree, and the claims did not survive: each is clean only at the commit its task tested it on, because the next commit added lines to a file the revert deletes. `2709fcec` conflicts from `b0c959fd` onward, `3a773dab`+`5bbc6f0a` from `3c6ee4cd` onward; `5bbc6f0a` alone is still clean today. The restore instruction is the durable procedure; the matrix says where a bare revert still applies.
- Corrections: `doc/main/cli/architecture.md`'s boundary table still listed `agent providers` and `agent summaries` as surfaces the server spawns; `doc/main/architecture.md` named six exported subpaths where there are eight; `doc/main/app/backend/libs.md` pointed at a renamed anchor; and `doc/main/cli/exports.md` said the emit pin fails on "any edit", where comments and type-only edits are normalized away — it now says any edit that changes the emitted program. The summary-read figures in the entry below were restated from `impl-summary-read-cutover-summary.md`, the artifact stamped at that task's final head; the entry had carried an earlier run of the same load-sensitive harness.

**Why:**
- The milestone's four cutovers are one decision applied four times, and the parts a future reader most needs are exactly the parts no single task owned: the shared rule, the comparison between cutovers, and the two negative results — the history read that was measured and deliberately not moved, and the single-file task store that reaches parity rather than improvement.
- Both negative results are the kind that quietly become wrong in documentation. "We didn't move it, and here is the measurement that says it should move in a bounded form" is the honest state, and it points at `history-read-land` rather than reading as an oversight.

**Key files:** `doc/main/cli/read-path.md` (new), `doc/main/cli/{README,architecture,exports,task,providers}.md`, `doc/main/architecture.md`, `doc/main/app/backend/{libs,routes}.md`, `doc/dev/cli/workflow.md`
**Verification:** doc-only diff — no file under `cli/`, `app/`, `scripts/` or `tools/` is touched. Every figure was re-read from the artifact that produced it (`plan/all/cli-node-sdk/{impl,qa}-*`) rather than copied from the task brief, and every code path the page names was checked against the source (the eight-entry exports map, the four in-process call sites, the four CLI commands that remain the rollback surface). The revert-at-head claims were replayed in a throwaway `git worktree`.
**Commit:** `80af2a26..HEAD`
**Next:** `history-read-land` — the bounded history reader the `history-read-cutover` measurement specified.
**Blockers:** None. Reported, not fixed: three comment-only wording edits under `cli/` remain from `summary-read-cutover`'s prepared follow-up (`plan/all/cli-node-sdk/followup-summary-read-cutover-wording.py`); the doc half of that follow-up landed here, the code half is out of this task's `doc/**` scope.

## 2026-08-11: session summaries and the provider catalog read in process

**What changed:**
- `app/server` no longer spawns `yaco agent summaries --path <p> --json` per project path with a cache miss, nor `yaco agent providers --json` before every `agent start`. Route wall p50 for the summary read: **122 ms → 30 ms** on the operator's real provider home, **138 ms → 36 ms** on a ten-times synthetic tree; p95 starvation of an already-queued timer **27.1 → 13.4 ms** and **34.3 → 14.1 ms**. `agent start` still spawns.
- New `cli/src/lib/core/agent/providers/summary-read.ts`, exported as `./core/agent/summaries`: `readSessionSummaries(targets) → Promise<Result<SessionSummary[]>>`. The sessions are explicit inputs — the app passes exactly the misses its cache is holding, the CLI passes its state files — so `session-state.ts` stays out of the closure and two concurrent calls on different projects cannot cross. `yaco agent summaries` is an argv-and-render adapter over the same function.
- What made the previous reader inadmissible was not being in process: it decoded and parsed each provider log **whole** to find a label that is almost always in the first record (241 ms on a 38 MB session log). The shared reader scans the head 256 KB at a time, parses only complete records, yields between steps, and stops at the first label — the same answer, because `firstMeaningfulMessage` judges each text independently of the ones around it.
- New `cli/src/lib/core/agent/provider-catalog.ts` on the `./core/agent` barrel. The adapters spread their `{id,label,executable}` out of it rather than the catalog restating what they declare, so the registry cannot disagree with the catalog by construction. The audit asserts the module reaches **no specifier at all** — so it can call no filesystem API — and reads no environment name.
- `RULE_5_SQLITE` in the export audit is rule 5's first *admission* (permanent, evidenced) as opposed to a *debt* (owed, retired): Codex's `threads` point lookup, isolated in `providers/codex-thread.ts`. It pins the SQL a reader can hold against the bound, and — the pin that means it — **the JavaScript the module compiles to**, checked in as a 22-line fixture, so any edit that changes the emitted program fails and has to be re-judged and re-measured (comments and type annotations are normalized away). Four versions that listed forbidden constructs were each found incomplete in review, the last because `(() => {}).constructor("… .all()")` runs a query the parser never sees; a fifth that summarized the syntax tree missed `const` vs `using`, which live in a declaration list's flags rather than in a child token. Pinning the emit has neither gap, and it is why the query has a module of its own. `summary-stall.ts --sqlite-probe` reproduces the plan and the 0.3 ms cost it cites.
- Two accepted behaviour changes, both recorded in `doc/main/cli/providers.md`, pulling in opposite directions and neither instantiated anywhere in the local corpus: a record over 4 MiB is skipped undecoded, and the Codex rollout search is the whole `YYYY/MM/DD` tree, continuing to the next day when a rollout yields no label. Which file a day contributes is unchanged.
- Deleted as production-dead: `TuiProvider.messages` (nothing has read it since the channel message cutover) and `ProviderHistory.summarize`. Both were shadows of a read-side registry the exported closures may not reach; their guards are now the stronger "every registered provider id has a reader", which needs no shadow field. Three copies of `userHome()` and two of `codexDbPath()` collapsed into `providers/provider-home.ts`; the label-collapsing rules moved to `providers/prompt-label.ts`, shared by the history list and the summary read.

**Why:**
- The design's Phase-2 cutovers 2 and 3, merged because both live under `cli/src/lib/core/agent/**` and would have collided if run in parallel.
- Every claim here is a measurement, and the harness is built to be falsifiable: `summary-stall.ts` carries a `whole-file` control route — the previous reader's shape through the same call mechanism — so a run that failed to separate bounded from unbounded would have said its own in-process figure was meaningless. It separates them by 10-50x at every scale.
- Eight High findings across five rounds of independent Codex review changed the code rather than the prose. Chunking bounds the scan but not one record, and a 36 MB record measured 96 ms — hence the cap, and a `--long-record` fixture that holds the route to the same bound. "Strictly more labels" was wrong: the newest rollout can open with nothing but a filtered context block while the real prompt sits in yesterday's, so the resolver became a lazy newest-first generator the summary read walks until a label. And the SQLite admission check was found incomplete five times over — a text match, a callee-name match, a property-access match, that plus a pinned import list, and finally a syntax-tree summary that could not see `const` vs `using` because the mode lives in a declaration list's flags. The first four each let a whole second unbounded query run while the audit reported exactly the admitted one. The lesson generalizes: a check that lists what code may not do is one construct behind, and a summary of code is only as faithful as the properties someone remembered to include. Pinning the emitted JavaScript has neither problem — it is what Node runs.

**Key files:** `cli/src/lib/core/agent/{provider-catalog.ts,index.ts}`, `cli/src/lib/core/agent/providers/{summary-read,codex-thread,prompt-label,provider-home,output,history,types,claude,codex}.ts`, `cli/src/commands/agent/{summaries,providers,index}.ts`, `cli/package.json`, `app/server/src/lib/{session-summary,agent}.ts`, `cli/test/{summary,providers,agent-messages-parity}.test.ts`, `cli/test/unit/export-audit.test.ts`, `cli/test/helpers/export-closure.ts`, `cli/test/fixtures/rule5-sqlite/`, `cli/test/bench/summary-{stall,fixture}.ts`, `doc/main/cli/{providers,exports}.md`, `doc/main/app/backend/{libs,routes}.md`, `doc/main/app/ui/workspace/sessions-and-terminal.md`, `app/CLAUDE.md`
**Verification:** `bash scripts/verify.sh` all 11 steps pass (1345 CLI unit tests, 859 app/server tests). Label parity on the operator's real provider home: **611/611** sessions (211 Claude logs across every project directory, 400 Codex threads) identical to the implementation this replaces. The CLI golden matrix is unchanged, so `agent summaries` and `agent providers` stdout/stderr/exit code are byte-identical to the pre-cutover baseline. Ten user flows end to end against isolated servers, including a stubbed `yaco` binary that proves the labels no longer come from a subprocess and that `agent start` still spawns. Reverting both feature commits applies cleanly and leaves every suite green — at the feature head where that was tested (`3a773dab`); from the next commit onward a bare revert conflicts, and the durable rollback is the call-site restoration recorded in `doc/main/cli/read-path.md`. See `plan/all/cli-node-sdk/{qa,review_codex}-summary-read-cutover.md`.
**Commit:** `5bbc6f0a..HEAD`
**Next:** `history-read-land` — the bounded history reader the `history-read-cutover` measurement specified.
**Blockers:** None. Reported, not fixed: in a git worktree, `scripts/worktree-provision.sh` symlinks `app/server/node_modules` to the main checkout's, so `@yaco/cli` resolves to the **main checkout's** source — 9 of 52 `app/server` test files failed here until it was corrected locally. The durable fix is outside this task's scope.
...
- The subprocess route is not a free baseline, which is the finding the verdict turns on. `spawn-noop` accounts for 37.6 of its 42.3 ms, and still costs 15.9 ms with the app's `ssh-add` discovery removed: `fork` with a loaded heap. Read that off the real home or `--scale 1`; the 10× spawn figures are confounded by the harness building its 550 MB fixture in the process it then measures, and review flagged them as such.
...
**Key files:** `cli/test/unit/export-audit.test.ts`, `cli/test/helpers/export-closure.ts`, `cli/test/fixtures/rule5-sqlite/`, `cli/src/lib/core/{task,worktree,paths}/index.ts`, `cli/src/commands/task/lock-timeout.ts`, `cli/src/lib/core/paths/toml.ts`, `doc/main/cli/exports.md`
