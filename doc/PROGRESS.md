...
- `RULE_5_SQLITE` in the export audit is rule 5's first *admission* (permanent, evidenced) as opposed to a *debt* (owed, retired): Codex's `threads` point lookup, isolated in `providers/codex-thread.ts`. It pins the SQL a reader can hold against the bound, and — the pin that means it — the module's **whole executable syntax**, checked in as a fixture, so any edit at all fails and has to be re-judged and re-measured. Four versions that enumerated dangerous spellings were each defeated in review, the last by `(() => {}).constructor("… .all()")`, which no list of banned names can see. Pinning the measured code ends that race, and it is why the query has a module of its own. `summary-stall.ts --sqlite-probe` reproduces the plan and the 0.3 ms cost it cites.
...
- Seven High findings across four rounds of independent Codex review changed the code rather than the prose. Chunking bounds the scan but not one record, and a 36 MB record measured 96 ms — hence the cap, and a `--long-record` fixture that holds the route to the same bound. "Strictly more labels" was wrong: the newest rollout can open with nothing but a filtered context block while the real prompt sits in yesterday's, so the resolver became a lazy newest-first generator the summary read walks until a label. And the SQLite admission check was fail-open four times over — a text match, a callee-name match, a property-access match, and that plus a pinned import list — each defeated by a spelling the previous version did not enumerate, and each running a whole second unbounded query while the audit reported exactly the admitted one. The lesson is the general one: a check that enumerates escapes is one escape behind. Pinning the syntax of the code that was measured has no escapes to enumerate.
...
**Verification:** `bash scripts/verify.sh` all 11 steps pass (1344 CLI unit tests, 859 app/server tests). Label parity on the operator's real provider home: **611/611** sessions (211 Claude logs across every project directory, 400 Codex threads) identical to the implementation this replaces. The CLI golden matrix is unchanged, so `agent summaries` and `agent providers` stdout/stderr/exit code are byte-identical to the pre-cutover baseline. Ten user flows end to end against isolated servers, including a stubbed `yaco` binary that proves the labels no longer come from a subprocess and that `agent start` still spawns. Reverting both feature commits applies cleanly and leaves every suite green. See `plan/all/cli-node-sdk/{qa,review_codex}-summary-read-cutover.md`.
