...
- **Design system tokens** in `ui/src/index.css`: `--font-ui` (Instrument Sans) for UI chrome, `--font-mono` for code/terminal. `--elevation-0` through `--elevation-3` for box-shadows (theme-aware). `--transition-fast`/`--transition-normal`/`--transition-slow` for timing. `--luminous-edge` gradient for the signature spectral top-border accent. All dialogs/menus/panels use `backdrop-filter: var(--backdrop-blur)` with semi-transparent backgrounds and entry animations (`dialog-enter`, `menu-enter`, `panel-slide-in`). Apply `.luminous-edge` class to elevated surface headers. Respect `prefers-reduced-motion`.
