...
9. **Voice input** → browser `MediaRecorder` captures audio → POST `/api/voice/compose` (multipart) → Groq Whisper STT → Groq LLM formatter → compose tray for user review → Insert (editor) or Send (terminal). Config: `GROQ_API_KEY` + optional model overrides in `server/.env`.
