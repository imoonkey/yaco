...
6. **Terminal** → WebSocket `/ws/terminal/:name?project=<projectName>` → node-pty (shell or tmux attach). Shell sessions keep a bounded in-memory scrollback buffer and are auto-reaped after 30 minutes with no attached clients; agent sessions use the global `~/.multmux/sessions/<handle>.json` state file where `handle` is the tmux session name
