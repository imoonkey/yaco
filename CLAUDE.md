...
6. **Terminal** → WebSocket `/ws/terminal/:name?project=<projectName>` → node-pty (shell) or tmux attach (agent sessions, project-scoped via state file `tmuxSession` field)
7. **Agent sessions** → `.multmux/*.json` state files → watched by project-watcher → SSE `sessions` channel
