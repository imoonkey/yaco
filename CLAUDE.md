...
5. **Terminal** → WebSocket → node-pty `tmux attach-session`. Shell sessions are Workflow-managed tmux sessions with ownership state in `~/.workflow/shell-sessions/*.json`; agent sessions use `~/.multmux/sessions/*.json`.
